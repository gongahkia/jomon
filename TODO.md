...
- [x] Define controls for region size, history length, climate, terrain and waterways, settlement and population density, political fragmentation, resource scarcity, ecology, dangers, era pace, and simulation fidelity; preserve every selected and resolved value in the manifest. `WorldManifest` v4 retains canonical selected settings, their full resolution, the existing generator version and normalized seed, accepted-validation diagnostics, four deterministic candidate streams, initial-world selection diagnostics, and content-safety provenance.
...
Implementation design for the active expanding-frontier contract:

- `src/medieval/frontier.ts` will own the versioned, renderer-independent frontier state, coordinate/identity helpers, regional-commitment and materialization planners, knowledge reducers, no-contradiction validator, and content-safety record collection. It consumes `InitialWorld` and resolved configuration as immutable inputs without changing the v4 world manifest or storage schema; the following manifest task will choose its durable envelope.
