...
An original isometric mini-golf party game with seed-shuffled courses, local hot-seat play, and up to four AI enemies.
...
Ordinary local play now starts in the `Party Rules` preset: a deliberately constrained nine-hole hot-seat campaign built to test the core social loop—**the shuffler creates the problem, the player creates the outcome, and the group creates the story**. Existing content remains available through the programmatic `custom` ruleset for experiments and legacy saves.

Party Rules uses three representative biomes—Speedway, Quarry, and Carnival—and three authored course grammars—Ribbon, Fork, and Courtyard. Every generated hole marks a safe line, a skill line, and a conflict line. Before each tee-off, the shuffler deterministically resolves that hole’s biome, layout, rules package, and one or two distinct chaos modifiers from the seed; players get the concise course headline but cannot steer or reroll it.
...
Each resolved hole records a versioned materialized recipe containing the generator version, seed, randomly selected course ingredients, chaos modifiers, content identifiers, route-role assignments, and a stable course hash. New `party-slice-v3` recipes add coherent terrain noise and cardinal course directions; recorded v1 and v2 recipes continue to use their historical geometry. Completed holes remain visibly stitched into the final course, while new appended holes prefer collision-free placements outside their shared cup-to-tee junction. Party Rules keeps gameplay systems active only on the current hole. The in-game intel drawer exposes the compact recipe hash and local development instrumentation records recipes, shot types, card use, collisions, recoveries, and per-hole strokes.

## Party Rules developer validation

The first-time guide is contextual, skippable, and stored only on the local device. It introduces the shared objective, automatic course shuffling, course-route roles, putt/chip input, visible Trick Card targets and expiries, social receipts, the three-card shop, and connected-course memory. Settings includes a local reset for the guide.
...
The server rejects unknown message shapes, rejects client transition-completion requests, verifies that a player owns each emote and active-turn action, bounds shot input, rate-limits sessions and room attempts, limits WebSocket payload size, and checks browser Origins against `APP_ORIGINS` before upgrade. It is intentionally a small private-room service rather than hosted matchmaking, accounts, spectators, anti-cheat, or voice.
