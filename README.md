...
Every match is a fixed nine-hole local hot-seat campaign. Before tee-off, each local player votes through a public three-card selection for every hole in the match; bots cast deterministic seeded-random ballots, plurality wins, and a seed-based tie-break settles any draw. The resulting compact, seeded match plan is locked before play starts—there is no loading screen between selection and the first shot. After each completed hole, the camera pulls back to a persistent campaign atlas: completed islands remain in place, a route extends from the previous island, and the next planned course assembles beside it before the camera returns for play. After hole nine, the final screen zooms all the way out over the completed route while leaving the podium and standings available in a compact foreground panel. Lowest aggregate strokes wins.
