...
- A reusable sandbox environment boundary with deterministic initial state, draw transitions, legal
  ...
  basic active/winning ippatsu metadata, basic closed-kan/ankan actions with simple live-wall
  replacement draws, individual reaction passes, ron-priority call gating, discard/call/tsumo/ron
  ...
  terminal-outcome plumbing, auto-passing reaction windows because it has no ron/call policy yet.
  It is not a full riichi/Sanma simulator, complete riichi declaration/accounting model, yaku
  validator, dead-wall/kan-dora/rinshan implementation, post-riichi closed-kan exception model,
  scoring engine, or RL implementation.
