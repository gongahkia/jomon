...
Mouse movement aims a human player's shot. By default, Space shoots, `-`/`=` change power, Enter locks a generator candidate, `R` rerolls candidates, and `P` uses a held chaos item. `?` opens the shortcut list and F1 opens settings, where all shortcuts can be remapped.
