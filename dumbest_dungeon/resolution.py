...
class Payload:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.card_copy_id) is not int or self.card_copy_id < 0:
            raise ValueError("invalid card-copy continuation identity")
        if any(value is not None and (not isinstance(value, str) or not value)
               for value in (self.actor_id, self.status, self.bonus_status, self.card_id,
                             self.card_mastery, self.card_infusion)):
...
class EventQueue:
    ...
    def from_snapshot(cls, snapshot: dict) -> EventQueue:
        ...
        try:
            if (type(raw["schema"]) is not int or raw["schema"] != 5 or any(type(raw[key]) is not int or raw[key] < 1 for key in ("next_event_id", "next_root_id"))
