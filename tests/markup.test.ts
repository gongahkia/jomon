...
describe('voting overlay markup', () => {
  ...
  it('shows the selected course package while its arena is assembling', () => {
    ...
    const markup = renderAppMarkup({ state, config: state.config, preferences: defaultPreferences(), overlay: undefined, drawer: undefined, aim: { angle: 0, power: 4 }, shotInFlight: false, assemblyProgress: .5, ledger: [], callouts: [] });
