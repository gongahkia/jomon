...
class CompleteEnemyRosterTests(unittest.TestCase):
    ...
    def test_roster_has_eighty_eight_standards_and_twenty_four_elites(self):
        validate_roster()
        report = roster_audit()
        self.assertEqual(report["standard_archetypes"], 88)
        self.assertEqual(report["mechanically_distinct_signatures"], 88)
        self.assertEqual(report["elite_situations"], 24)
        self.assertEqual(report["named_recurring_rivals"], 8)
        self.assertEqual(set(report["regions"].values()), {11})
    ...
    def test_each_expanded_role_has_a_production_group_and_tested_reducer(self):
        ...
        for row in EXPANDED_STANDARD_ACTORS:
            ...
            seen.add(region)
        self.assertEqual(len(EXPANDED_STANDARD_ACTORS), 40)
        self.assertEqual(len(seen), 8)

    def test_all_four_new_hearthford_roles_appear_in_ordinary_seeded_worlds(self):
        seen = set()
