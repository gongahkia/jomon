...
class FourPlayerScoringInvariantTests(unittest.TestCase):
    def test_generated_ron_and_chankan_ledgers_are_conservative(self) -> None:
        for fixture in _score_fixtures():
            with self.subTest(fixture=fixture):
                ron = score_riichi_hand(**fixture, win_kind="ron", players=4)
                chankan = score_riichi_hand(**fixture, win_kind="chankan", players=4)
                ...
                self.assertEqual(ron.tsumo_child_payment, None)
                self.assertEqual(ron.tsumo_dealer_payment, None)

    def test_generated_tsumo_ledgers_conserve_loser_payments(self) -> None:
        for fixture in _score_fixtures():
            with self.subTest(fixture=fixture):
                result = score_riichi_hand(**fixture, win_kind="tsumo", players=4)
