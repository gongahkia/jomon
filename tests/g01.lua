...
function T.run()
 ...
 group('G01 torches can mount to solid wall faces and retain real light support',function()
  local c=modern();local w=c.sites[1].world;local context={campaign=c,siteId=1};local gx,gy=10,6
  local x1,y1,x2,y2=W.rect(gx,gy);F.fill(w,x1,y1,x2,y2,M.AIR);F.fill(w,x1,y2+1,x2,y2+1,M.AIR);F.fill(w,x1-1,y1,x1-1,y2,M.ROCK)
  local clear,why=Structures.siteClear(w,gx,gy,'torch');check(clear,why)
  local torch=Structures.install(w,gx,gy,'torch');eq(Structures.torchMount(w,gx,gy),'left');check(Structures.supported(w,torch),'Wall-mounted torch was not structurally supported')
  Visibility.update(w,context);local tx,ty=torch.gx*4-2,torch.gy*4-2;check(Visibility.lightAt(w,tx,ty,context)>0,'Wall-mounted torch emitted no light')
  F.fill(w,x1-1,y1,x1-1,y2,M.AIR);check(not Structures.supported(w,torch),'Torch stayed supported after backing wall vanished')
  w.structures[W.slot(w,gx,gy)]=nil;Structures.install(w,gx-1,gy,'wall')
  clear,why=Structures.siteClear(w,gx,gy,'torch');check(clear,why)
  local wallTorch=Structures.install(w,gx,gy,'torch');eq(Structures.torchMount(w,gx,gy),'left');check(Structures.supported(w,wallTorch),'Completed wall did not support a mounted torch')
 end)
