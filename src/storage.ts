...
const isRunTelemetry = (value: unknown): value is RunTelemetry => isRecord(value) && isNumber(value.turns) && isRunActions(value.actions) && isNumber(value.kills) && isNumber(value.damageDealt) && isNumber(value.damageTaken) && isNumber(value.goldGained) && (value.goldSpent === undefined || isNumber(value.goldSpent)) && isNumber(value.xpGained) && isNumber(value.pickups) && isNumber(value.bombsUsed) && isNumber(value.ropesUsed) && ['itemsUsed', 'boonPicks', 'boonAugments', 'relicPicks', 'purchases', 'enemyKills', 'eventOutcomes', 'deathCauses'].every(key => value[key] === undefined || isCounterMap(value[key])) && Array.isArray(value.samples) && value.samples.every(isRunMetricSample) && Array.isArray(value.floors) && value.floors.every(isRunFloorMetrics)
...
const isFloorBase = (value: unknown): value is Omit<Floor, 'milestones' | 'transientTerrain'> & Partial<Pick<Floor, 'milestones' | 'transientTerrain'>> => isRecord(value) && isInteger(value.index) && oneOf(value.biome, BIOMES) && isNumber(value.seed) && isNonNegativeInteger(value.width) && value.width > 0 && isNonNegativeInteger(value.height) && value.height > 0 && isString(value.layoutId) && Array.isArray(value.tiles) && value.tiles.length === value.width * value.height && value.tiles.every(isTile) && Array.isArray(value.actors) && value.actors.every(isActor) && Array.isArray(value.items) && value.items.every(isGroundItem) && Array.isArray(value.props) && value.props.every(isProp) && (value.encounters === undefined || (Array.isArray(value.encounters) && value.encounters.every(isFloorEncounter))) && isPoint(value.start) && isPoint(value.exit) && typeof value.guardianDefeated === 'boolean' && isObjective(value.objective) && (value.ecology === undefined || (Array.isArray(value.ecology) && value.ecology.every(isEcologyEvent))) && (value.telegraphs === undefined || (Array.isArray(value.telegraphs) && value.telegraphs.every(isTelegraph))) && (value.puzzleIds === undefined || (Array.isArray(value.puzzleIds) && value.puzzleIds.every(isString))) && (value.climbLinks === undefined || (Array.isArray(value.climbLinks) && value.climbLinks.every(isClimbLink))) && (value.difficulty === undefined || isDifficulty(value.difficulty))
...
const validPersistedRun = (run: RunState): boolean => {
  ...
  if (floor.actors.some(actor => !pointOnMap(floor, actor)) || floor.items.some(item => !pointOnMap(floor, item)) || floor.props.some(prop => !pointOnMap(floor, prop) || (prop.effectCells?.some(point => !pointOnMap(floor, point)) ?? false)) || (floor.encounters?.some(encounter => !pointOnMap(floor, encounter)) ?? false) || (floor.ecology?.some(ecology => !pointOnMap(floor, ecology.target)) ?? false)) return false
...
export const migrateRunRecord = (value: unknown): RunState | undefined => {
  ...
  for (const key of ['itemsUsed', 'boonPicks', 'boonAugments', 'relicPicks', 'purchases', 'enemyKills', 'eventOutcomes', 'deathCauses'] as const) telemetry[key] ??= {}
  run.hero.relics ??= []
