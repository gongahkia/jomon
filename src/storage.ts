...
import { FLOOR_COUNT, MAP_HEIGHT, MAP_WIDTH, MONSTER_ROLES, floorIndex, inFloorBounds } from './types'
...
const isFloorBase = (value: unknown): value is Omit<Floor, 'milestones' | 'transientTerrain'> & Partial<Pick<Floor, 'milestones' | 'transientTerrain'>> => isRecord(value) && isInteger(value.index) && oneOf(value.biome, BIOMES) && isNumber(value.seed) && isNonNegativeInteger(value.width) && value.width > 0 && isNonNegativeInteger(value.height) && value.height > 0 && isString(value.layoutId) && Array.isArray(value.tiles) && value.tiles.length === value.width * value.height && value.tiles.every(isTile) && Array.isArray(value.actors) && value.actors.every(isActor) && Array.isArray(value.items) && value.items.every(isGroundItem) && Array.isArray(value.props) && value.props.every(isProp) && (value.encounters === undefined || (Array.isArray(value.encounters) && value.encounters.every(isFloorEncounter))) && isPoint(value.start) && isPoint(value.exit) && typeof value.guardianDefeated === 'boolean' && isObjective(value.objective) && (value.telegraphs === undefined || (Array.isArray(value.telegraphs) && value.telegraphs.every(isTelegraph))) && (value.puzzleIds === undefined || (Array.isArray(value.puzzleIds) && value.puzzleIds.every(isString))) && (value.climbLinks === undefined || (Array.isArray(value.climbLinks) && value.climbLinks.every(isClimbLink))) && (value.difficulty === undefined || isDifficulty(value.difficulty))
...
type LegacyFloor = Omit<Floor, 'width' | 'height' | 'layoutId' | 'props' | 'objective' | 'milestones' | 'transientTerrain'> & { props?: Floor['props']; objective?: Floor['objective']; milestones?: Floor['milestones']; transientTerrain?: Floor['transientTerrain'] }
...
const migrateLegacyRun = (legacy: LegacyRunState): RunState => ({
  version: 5,
  seed: legacy.seed,
  floor: { ...legacy.floor, width: MAP_WIDTH, height: MAP_HEIGHT, layoutId: 'legacy', props: legacy.floor.props ?? [], objective: legacy.floor.objective ?? objectiveForFloor(legacy.floor.index), milestones: legacy.floor.milestones ?? [], transientTerrain: legacy.floor.transientTerrain },
...
const validPersistedRun = (run: RunState): boolean => {
  ...
  if (floor.actors.some(actor => !pointOnMap(floor, actor)) || floor.items.some(item => !pointOnMap(floor, item)) || floor.props.some(prop => !pointOnMap(floor, prop) || (prop.effectCells?.some(point => !pointOnMap(floor, point)) ?? false)) || (floor.encounters?.some(encounter => !pointOnMap(floor, encounter)) ?? false)) return false
