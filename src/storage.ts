...
const isFloorEscalation = (value: unknown): value is FloorEscalation => isRecord(value) && isString(value.arcId) && isExpeditionPhase(value.phase) && ['topology', 'landmark', 'encounter', 'ecology', 'promise', 'payoff'].every(key => isString(value[key])) && isNumber(value.encounterOffset) && Array.isArray(value.carried) && value.carried.every(isExpeditionPhase)
...
const isFloorBase = (value: unknown): value is Omit<Floor, 'milestones' | 'transientTerrain'> & Partial<Pick<Floor, 'milestones' | 'transientTerrain'>> => isRecord(value) && isInteger(value.index) && oneOf(value.biome, BIOMES) && isNumber(value.seed) && isNonNegativeInteger(value.width) && value.width > 0 && isNonNegativeInteger(value.height) && value.height > 0 && isString(value.layoutId) && Array.isArray(value.tiles) && value.tiles.length === value.width * value.height && value.tiles.every(isTile) && Array.isArray(value.actors) && value.actors.every(isActor) && Array.isArray(value.items) && value.items.every(isGroundItem) && Array.isArray(value.props) && value.props.every(isProp) && (value.encounters === undefined || (Array.isArray(value.encounters) && value.encounters.every(isFloorEncounter))) && isPoint(value.start) && isPoint(value.exit) && typeof value.guardianDefeated === 'boolean' && isObjective(value.objective) && (value.escalation === undefined || isFloorEscalation(value.escalation)) && (value.ecology === undefined || (Array.isArray(value.ecology) && value.ecology.every(isEcologyEvent))) && (value.telegraphs === undefined || (Array.isArray(value.telegraphs) && value.telegraphs.every(isTelegraph))) && (value.puzzleIds === undefined || (Array.isArray(value.puzzleIds) && value.puzzleIds.every(isString))) && (value.climbLinks === undefined || (Array.isArray(value.climbLinks) && value.climbLinks.every(isClimbLink))) && (value.difficulty === undefined || isDifficulty(value.difficulty))
