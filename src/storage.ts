...
const isGroundItem = (value: unknown): value is GroundItem => isRecord(value) && (value.id === 'gold' || isItemId(value.id)) && isInteger(value.x) && isInteger(value.y) && isNonNegativeInteger(value.count) && value.count > 0 && (value.tool === undefined || isTraversalTool(value.tool))
