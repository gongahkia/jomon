...
import type { Actor, Alignment, AreaArcState, Biome, CampaignCarryoverDiagnostic, CampaignCycle, CampaignRouteState, ClimbLink, Companion, ConditionState, CourierCalling, CourierMenuEntry, CourierOrigin, CourierSave, CurseState, DeathMode, DifficultyContext, EncounterKind, EncyclopediaState, Floor, FloorEncounter, FloorEscalation, FloorMilestone, GroundItem, Hero, InteractionTelemetry, LegacyRecord, LineageEvent, Modal, OathState, OptionalContentTelemetry, Point, Prop, Records, RelicId, RescuedNpc, RewardOffer, RunAnalysis, RunFloorMetrics, RunMetricSample, RunState, RunTelemetry, SecretClueChannel, SecretRoom, SecretRoute, ShortcutReturn, SocialContract, SocialReputation, Telegraph, Tile, TransientTerrain, TraversalToolId } from './types'
...
export const migrateCampaignRoute = (value: unknown): CampaignRouteState => {
  ...
  const companions = value.companions === undefined ? companionLeadsForRescues(rescuedNpcs) : cloneCompanions(value.companions, rescuedNpcs)
  return { version: 5, areaOrder: value.version === 4 || value.version === 5 ? [...value.areaOrder!] : value.version === 3 ? [...value.areaOrder!] : [...LEGACY_AREA_ORDER], completedAreas: [...value.completedAreas], unlockedAreas: [...value.unlockedAreas], selectedBiome: value.selectedBiome, rescuedNpcs, companions, carryoverDiagnostics: cloneCarryoverDiagnostics(value.carryoverDiagnostics ?? []), lineageEvents: (value.lineageEvents ?? []).map(event => ({ ...event })), legacyRecords: copyLegacyRecords(value.legacyRecords ?? []), alignment: { kami: value.alignment?.kami ?? 0, villagePact: value.alignment?.villagePact ?? 0 }, reputation: { trailfolk: value.reputation?.trailfolk ?? 0, kami: value.reputation?.kami ?? 0 }, cycle: value.cycle ? cloneCampaignCycle(value.cycle) : initialCampaignCycle() }
}
