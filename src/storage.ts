...
import type { Actor, Alignment, Biome, CampaignRouteState, ClimbLink, ConditionState, CourierCalling, CourierMenuEntry, CourierOrigin, CourierSave, CurseState, DeathMode, DifficultyContext, EncounterKind, EncyclopediaState, Floor, FloorEncounter, FloorMilestone, GroundItem, Hero, LegacyRecord, LineageEvent, Modal, OathState, Point, Prop, Records, RelicId, RescuedNpc, RunAnalysis, RunFloorMetrics, RunMetricSample, RunState, RunTelemetry, Telegraph, Tile, TransientTerrain, TraversalToolId } from './types'
...
const isCampaignRoute = (value: unknown): value is CampaignRouteRecord => isRecord(value) && (value.version === 1 || value.version === 2 || value.version === 3 || value.version === 4 || value.version === 5) && Array.isArray(value.completedAreas) && value.completedAreas.every(area => oneOf(area, BIOMES)) && Array.isArray(value.unlockedAreas) && value.unlockedAreas.every(area => oneOf(area, BIOMES)) && oneOf(value.selectedBiome, BIOMES) && value.unlockedAreas.includes(value.selectedBiome) && (value.version === 3 ? Array.isArray(value.areaOrder) && isLegacyCampaignAreaOrder(value.areaOrder as Biome[]) : value.version === 4 || value.version === 5 ? Array.isArray(value.areaOrder) && isCampaignAreaOrder(value.areaOrder as Biome[]) : true) && (value.alignment === undefined || isAlignment(value.alignment)) && (value.rescuedNpcs === undefined || (Array.isArray(value.rescuedNpcs) && value.rescuedNpcs.every(isRescuedNpc))) && (value.lineageEvents === undefined || (Array.isArray(value.lineageEvents) && value.lineageEvents.every(isLineageEvent))) && (value.legacyRecords === undefined || (Array.isArray(value.legacyRecords) && value.legacyRecords.every(isLegacyRecord)))
...
export const migrateCampaignRoute = (value: unknown): CampaignRouteState => isCampaignRoute(value) ? { version: 5, areaOrder: value.version === 4 || value.version === 5 ? [...value.areaOrder!] : value.version === 3 ? [...value.areaOrder!] : [...LEGACY_AREA_ORDER], completedAreas: [...value.completedAreas], unlockedAreas: [...value.unlockedAreas], selectedBiome: value.selectedBiome, rescuedNpcs: (value.rescuedNpcs ?? []).map(npc => ({ ...npc })), lineageEvents: (value.lineageEvents ?? []).map(event => ({ ...event })), legacyRecords: copyLegacyRecords(value.legacyRecords ?? []), alignment: { kami: value.alignment?.kami ?? 0, villagePact: value.alignment?.villagePact ?? 0 } } : initialCampaignRoute()
