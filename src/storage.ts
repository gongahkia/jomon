...
const isObjective = (value: unknown): boolean => isRecord(value) && isString(value.id) && oneOf(value.kind, ['recoverSupplies', 'rescueScout', 'invokeAltar', 'defeatGuardian', 'stabilizeIntake']) && oneOf(value.status, ['active', 'complete']) && isString(value.label)
...
const isTelegraph = (value: unknown): value is Telegraph => isRecord(value) && isString(value.id) && isString(value.sourceId) && isString(value.actionId) && Array.isArray(value.cells) && value.cells.every(isPoint) && oneOf(value.danger, ['minor', 'major']) && isNumber(value.resolveTurn) && (value.declaredTurn === undefined || isNonNegativeInteger(value.declaredTurn)) && (value.sourceKind === undefined || oneOf(value.sourceKind, ['actor', 'hazard'])) && (value.category === undefined || oneOf(value.category, ['enemyAttack', 'pressureVent', 'relayDischarge', 'intakeShear'])) && (value.responses === undefined || (Array.isArray(value.responses) && value.responses.every(response => oneOf(response, ['move', 'interrupt', 'ground', 'block'])))) && (value.state === undefined || oneOf(value.state, ['pending', 'resolved', 'cancelled'])) && (value.collision === undefined || (isRecord(value.collision) && isPoint(value.collision.point) && isString(value.collision.by))) && (value.cover === undefined || typeof value.cover === 'boolean')
const isTacticalIntentHistory = (value: unknown): boolean => isRecord(value) && isString(value.id) && oneOf(value.state, ['resolved', 'cancelled']) && isNonNegativeInteger(value.turn)
const isDeliveryExpedition = (value: unknown): boolean => isRecord(value) && value.version === 1 && isString(value.id) && value.kind === 'nerida-intake' && isString(value.runId) && value.objective === 'stabilize-intake' && oneOf(value.status, ['active', 'completed', 'escaped']) && isPoint(value.console) && Array.isArray(value.hazardIds) && value.hazardIds.every(isString) && typeof value.modificationAvailable === 'boolean' && (value.relayActivated === undefined || typeof value.relayActivated === 'boolean') && (value.lastHeroPosition === undefined || isPoint(value.lastHeroPosition)) && (value.repeatedHeading === undefined || isString(value.repeatedHeading)) && (value.repeatedHeadingCount === undefined || isNonNegativeInteger(value.repeatedHeadingCount)) && (value.techniqueGranted === undefined || typeof value.techniqueGranted === 'boolean')
const isDeliveryContext = (value: unknown): boolean => isRecord(value) && value.version === 1 && isString(value.runId) && oneOf(value.pressureTier, ['working-load', 'compression', 'cavitation', 'cascade']) && isNonNegativeInteger(value.elapsedMarks) && (value.atRouteReckoning === undefined || isNonNegativeInteger(value.atRouteReckoning)) && (value.nextThresholdMark === undefined || isNonNegativeInteger(value.nextThresholdMark))
...
type LegacyRunState = Omit<RunState, 'version' | 'floor' | 'hero'> & { version: 1 | 2 | 3 | 4 | 5 | 6; floor: LegacyFloor; hero: LegacyHero | Hero }
...
const isLegacyRunState = (value: unknown): value is LegacyRunState => isRecord(value) && (value.version === 1 || value.version === 2 || value.version === 3 || value.version === 4 || value.version === 5 || value.version === 6) && isNumber(value.seed) && isLegacyFloor(value.floor) && (isLegacyHero(value.hero) || isHero(value.hero)) && Array.isArray(value.messages) && value.messages.every(isString) && oneOf(value.status, ['title', 'playing', 'dead', 'victory']) && isModal(value.modal) && isNonNegativeInteger(value.turn) && (value.area === undefined || oneOf(value.area, BIOMES)) && (value.areaFloor === undefined || isInteger(value.areaFloor))
...
export const migrateRunRecord = (value: unknown): RunState | undefined => {
  ...
  const run = isRunState(migrated) ? { ...migrated } : isLegacyRunState(migrated) && migrated.version < 6 ? migrateLegacyRun(migrated) : undefined
  ...
  if (run.replayHero) run.replayHero = structuredClone(run.replayHero)
  run.companions = cloneCompanions(run.companions ?? [])
  run.companionDeathMode ??= 'injury'
  run.version = 6
  run.hero.deliveryModifications = (run.hero.deliveryModifications ?? []).filter(modification => modification.id === 'pressure-baffles' || modification.id === 'relay-marrow-conduit').slice(-2)
  run.hero.deliveryInjuries = (run.hero.deliveryInjuries ?? []).filter(injury => injury.id === 'pressure-scarring').slice(-3)
  run.hero.learnedDeliveryTechniques = (run.hero.learnedDeliveryTechniques ?? []).filter(technique => technique === 'intake-routing').slice(-3)
  if (run.floor.tacticalIntentHistory) run.floor.tacticalIntentHistory = run.floor.tacticalIntentHistory.filter(intent => intent.state === 'resolved' || intent.state === 'cancelled').slice(-16)
  if (run.floor.deliveryExpedition) run.floor.deliveryExpedition.relayActivated ??= false
  if (run.deliveryContext) run.deliveryContext.atRouteReckoning ??= run.deliveryContext.elapsedMarks
  return run
}
