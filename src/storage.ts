...
import type { Actor, Biome, CampaignRouteState, ClimbLink, ConditionState, CourierCalling, CourierMenuEntry, CourierOrigin, CourierSave, CurseState, DeathMode, DifficultyContext, EncounterKind, EncyclopediaState, Floor, FloorEncounter, FloorMilestone, GroundItem, Hero, LegacyRecord, LineageEvent, Modal, OathState, Point, Prop, Records, RelicId, RescuedNpc, RunAnalysis, RunFloorMetrics, RunMetricSample, RunState, RunTelemetry, Telegraph, Tile, TransientTerrain, TraversalToolId } from './types'
...
const BIOMES = ['mine', 'wilds', 'caverns', 'ruins', 'furnace', 'floodedRuins', 'cliffs', 'burial'] as const
...
const TILE_KINDS = ['wall', 'floor', 'exit', 'door', 'lockedDoor', 'water', 'lava', 'pit', 'rope', 'spikes', 'dart', 'fireVent', 'crumble', 'boulder', 'web', 'gas', 'support', 'rail', 'rubble', 'bramble', 'darkness', 'crate', 'chest', 'altar', 'shop', 'rescue', 'smoke', 'lift', 'breakwall', 'current', 'deepWater', 'anchor', 'cliffWall', 'ledge', 'graveSoil', 'cairn', 'ossuary', 'spiritPath'] as const
const isPoint = (value: unknown): value is Point => isRecord(value) && isNumber(value.x) && isNumber(value.y)
const isTile = (value: unknown): value is Tile => isRecord(value) && oneOf(value.kind, TILE_KINDS) && typeof value.explored === 'boolean' && typeof value.visible === 'boolean' && (value.elevation === undefined || value.elevation === 0 || value.elevation === 1)
...
const isEncounterKind = (value: unknown): value is EncounterKind => oneOf(value, ['wayfarer', 'bloodBargain', 'shiftingChamber', 'stormCache', 'ancestorDebt', 'cursedObject', 'oathwell', 'windTrial', 'tombAuction'])
const isFloorEncounter = (value: unknown): value is FloorEncounter => isRecord(value) && isString(value.id) && isEncounterKind(value.kind) && isPoint(value) && oneOf(value.state, ['dormant', 'resolved'])
...
const isRunTelemetry = (value: unknown): value is RunTelemetry => isRecord(value) && isNumber(value.turns) && isRunActions(value.actions) && isNumber(value.kills) && isNumber(value.damageDealt) && isNumber(value.damageTaken) && isNumber(value.goldGained) && (value.goldSpent === undefined || isNumber(value.goldSpent)) && isNumber(value.xpGained) && isNumber(value.pickups) && isNumber(value.bombsUsed) && isNumber(value.ropesUsed) && ['itemsUsed', 'boonPicks', 'boonAugments', 'relicPicks', 'purchases', 'enemyKills', 'eventOutcomes'].every(key => value[key] === undefined || isCounterMap(value[key])) && Array.isArray(value.samples) && value.samples.every(isRunMetricSample) && Array.isArray(value.floors) && value.floors.every(isRunFloorMetrics)
...
const isMilestone = (value: unknown): value is FloorMilestone => isRecord(value) && isString(value.id) && oneOf(value.kind, ['waycache', 'boon', 'augment', 'relic']) && isPoint(value) && typeof value.discovered === 'boolean' && typeof value.claimed === 'boolean'
const isTransientTerrain = (value: unknown): value is TransientTerrain => isRecord(value) && isPoint(value) && oneOf(value.original, TILE_KINDS) && isNonNegativeInteger(value.expiresAt)
...
const isDifficulty = (value: unknown): value is DifficultyContext => isRecord(value) && isNonNegativeInteger(value.routePosition) && isNonNegativeInteger(value.threat) && isNumber(value.healthMultiplier) && isNonNegativeInteger(value.attackBonus) && isNonNegativeInteger(value.defenseBonus) && isNonNegativeInteger(value.eliteChance) && isNonNegativeInteger(value.guardianPattern)
const isFloorBase = (value: unknown): value is Omit<Floor, 'milestones' | 'transientTerrain'> & Partial<Pick<Floor, 'milestones' | 'transientTerrain'>> => isRecord(value) && isInteger(value.index) && oneOf(value.biome, BIOMES) && isNumber(value.seed) && Array.isArray(value.tiles) && value.tiles.length === MAP_WIDTH * MAP_HEIGHT && value.tiles.every(isTile) && Array.isArray(value.actors) && value.actors.every(isActor) && Array.isArray(value.items) && value.items.every(isGroundItem) && Array.isArray(value.props) && value.props.every(isProp) && (value.encounters === undefined || (Array.isArray(value.encounters) && value.encounters.every(isFloorEncounter))) && isPoint(value.start) && isPoint(value.exit) && typeof value.guardianDefeated === 'boolean' && isObjective(value.objective) && (value.telegraphs === undefined || (Array.isArray(value.telegraphs) && value.telegraphs.every(isTelegraph))) && (value.puzzleIds === undefined || (Array.isArray(value.puzzleIds) && value.puzzleIds.every(isString))) && (value.climbLinks === undefined || (Array.isArray(value.climbLinks) && value.climbLinks.every(isClimbLink))) && (value.difficulty === undefined || isDifficulty(value.difficulty))
...
const isTraversalTool = (value: unknown): value is TraversalToolId => oneOf(value, ['stoneWedge', 'reedwing', 'cordAnchor', 'ashwayRites'])
const isRelic = (value: unknown): value is RelicId => oneOf(value, ['ashCircuit', 'markbreakerSeal', 'cairnCoil', 'tideFetter'])
...
const isModal = (value: unknown): value is Modal | undefined => {
  ...
  return value.kind === 'target' && oneOf(value.action, ['throw', 'spell', 'bomb', 'drill', 'glide', 'grapple', 'bridge', 'dash', 'winch', 'stoneWedge', 'reedwing', 'cordAnchor', 'ashwayRites']) && (value.item === undefined || isItemId(value.item)) && (value.tool === undefined || isTraversalTool(value.tool)) && (value.overdrive === undefined || typeof value.overdrive === 'boolean')
}

const isRunRecord = (value: unknown): value is RunRecord => isRecord(value) && isNumber(value.version) && isNumber(value.seed) && isFloor(value.floor) && isHero(value.hero) && Array.isArray(value.messages) && value.messages.every(isString) && oneOf(value.status, ['title', 'playing', 'dead', 'victory']) && isModal(value.modal) && isNumber(value.turn) && (value.area === undefined || oneOf(value.area, BIOMES)) && (value.areaFloor === undefined || isNumber(value.areaFloor)) && (value.areaOrder === undefined || isAreaOrder(value.areaOrder)) && (value.gateDestination === undefined || oneOf(value.gateDestination, BIOMES)) && (value.rescuedNpcs === undefined || (Array.isArray(value.rescuedNpcs) && value.rescuedNpcs.every(isRescuedNpc))) && (value.lineageEvents === undefined || (Array.isArray(value.lineageEvents) && value.lineageEvents.every(isLineageEvent))) && (value.encyclopedia === undefined || isEncyclopedia(value.encyclopedia)) && (value.telemetry === undefined || isRunTelemetry(value.telemetry))
const isRunState = (value: unknown): value is RunState => isRunRecord(value) && (value.version === 4 || value.version === 5)
const isCampaignRoute = (value: unknown): value is CampaignRouteRecord => isRecord(value) && (value.version === 1 || value.version === 2 || value.version === 3 || value.version === 4) && Array.isArray(value.completedAreas) && value.completedAreas.every(area => oneOf(area, BIOMES)) && Array.isArray(value.unlockedAreas) && value.unlockedAreas.every(area => oneOf(area, BIOMES)) && oneOf(value.selectedBiome, BIOMES) && value.unlockedAreas.includes(value.selectedBiome) && (value.version === 3 ? Array.isArray(value.areaOrder) && isLegacyCampaignAreaOrder(value.areaOrder as Biome[]) : value.version === 4 ? Array.isArray(value.areaOrder) && isCampaignAreaOrder(value.areaOrder as Biome[]) : true) && (value.rescuedNpcs === undefined || (Array.isArray(value.rescuedNpcs) && value.rescuedNpcs.every(isRescuedNpc))) && (value.lineageEvents === undefined || (Array.isArray(value.lineageEvents) && value.lineageEvents.every(isLineageEvent))) && (value.legacyRecords === undefined || (Array.isArray(value.legacyRecords) && value.legacyRecords.every(isLegacyRecord)))

type LegacyHero = Omit<Hero, 'name' | 'origin' | 'calling' | 'deathMode'>
type LegacyFloor = Omit<Floor, 'props' | 'objective' | 'milestones' | 'transientTerrain'> & { props?: Floor['props']; objective?: Floor['objective']; milestones?: Floor['milestones']; transientTerrain?: Floor['transientTerrain'] }
type LegacyRunState = Omit<RunState, 'version' | 'floor' | 'hero'> & { version: 1 | 2 | 3; floor: LegacyFloor; hero: LegacyHero | Hero }
...
const isLegacyRunState = (value: unknown): value is LegacyRunState => isRecord(value) && (value.version === 1 || value.version === 2 || value.version === 3) && isNumber(value.seed) && isLegacyFloor(value.floor) && (isLegacyHero(value.hero) || isHero(value.hero)) && Array.isArray(value.messages) && value.messages.every(isString) && oneOf(value.status, ['title', 'playing', 'dead', 'victory']) && isModal(value.modal) && isNonNegativeInteger(value.turn) && (value.area === undefined || oneOf(value.area, BIOMES)) && (value.areaFloor === undefined || isInteger(value.areaFloor))
const migrateLegacyRun = (legacy: LegacyRunState): RunState => ({
  version: 5,
  seed: legacy.seed,
  floor: { ...legacy.floor, props: legacy.floor.props ?? [], objective: legacy.floor.objective ?? objectiveForFloor(legacy.floor.index), milestones: legacy.floor.milestones ?? [], transientTerrain: legacy.floor.transientTerrain },
...
const validPersistedRun = (run: RunState): boolean => {
  ...
  if (floor.actors.some(actor => !pointOnMap(actor)) || floor.items.some(item => !pointOnMap(item)) || floor.props.some(prop => !pointOnMap(prop) || (prop.effectCells?.some(point => !pointOnMap(point)) ?? false)) || (floor.encounters?.some(encounter => !pointOnMap(encounter)) ?? false)) return false
...
export const migrateRunRecord = (value: unknown): RunState | undefined => {
  ...
  for (const key of ['itemsUsed', 'boonPicks', 'boonAugments', 'relicPicks', 'purchases', 'enemyKills', 'eventOutcomes'] as const) telemetry[key] ??= {}
  run.hero.relics ??= []
...
export const migrateCampaignRoute = (value: unknown): CampaignRouteState => isCampaignRoute(value) ? { version: value.version === 4 ? 4 : 3, areaOrder: value.version === 4 ? [...value.areaOrder!] : value.version === 3 ? [...value.areaOrder!] : [...LEGACY_AREA_ORDER], completedAreas: [...value.completedAreas], unlockedAreas: [...value.unlockedAreas], selectedBiome: value.selectedBiome, rescuedNpcs: (value.rescuedNpcs ?? []).map(npc => ({ ...npc })), lineageEvents: (value.lineageEvents ?? []).map(event => ({ ...event })), legacyRecords: copyLegacyRecords(value.legacyRecords ?? []) } : initialCampaignRoute()
