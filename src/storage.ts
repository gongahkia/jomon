...
const isMilestone = (value: unknown): value is FloorMilestone => isRecord(value) && isString(value.id) && oneOf(value.kind, ['waycache', 'boon']) && isPoint(value) && typeof value.discovered === 'boolean' && typeof value.claimed === 'boolean'
const isTransientTerrain = (value: unknown): value is TransientTerrain => isRecord(value) && isPoint(value) && oneOf(value.original, ['wall', 'floor', 'exit', 'door', 'lockedDoor', 'water', 'lava', 'pit', 'rope', 'spikes', 'dart', 'fireVent', 'crumble', 'boulder', 'web', 'gas', 'support', 'rail', 'rubble', 'bramble', 'darkness', 'crate', 'chest', 'altar', 'shop', 'rescue']) && isNonNegativeInteger(value.expiresAt)
const isFloorBase = (value: unknown): value is Omit<Floor, 'milestones' | 'transientTerrain'> & Partial<Pick<Floor, 'milestones' | 'transientTerrain'>> => isRecord(value) && isInteger(value.index) && oneOf(value.biome, ['mine', 'wilds', 'caverns', 'ruins']) && isNumber(value.seed) && Array.isArray(value.tiles) && value.tiles.length === MAP_WIDTH * MAP_HEIGHT && value.tiles.every(isTile) && Array.isArray(value.actors) && value.actors.every(isActor) && Array.isArray(value.items) && value.items.every(isGroundItem) && Array.isArray(value.props) && value.props.every(isProp) && isPoint(value.start) && isPoint(value.exit) && typeof value.guardianDefeated === 'boolean' && isObjective(value.objective) && (value.telegraphs === undefined || (Array.isArray(value.telegraphs) && value.telegraphs.every(isTelegraph))) && (value.puzzleIds === undefined || (Array.isArray(value.puzzleIds) && value.puzzleIds.every(isString)))
