...
const isTraversalTool = (value: unknown): value is TraversalToolId => oneOf(value, ['stoneWedge', 'reedwing', 'cordAnchor', 'ashwayRites', 'antlerPrybar', 'stoneAdze'])
...
const isModal = (value: unknown): value is Modal | undefined => {
  ...
  return value.kind === 'target' && oneOf(value.action, ['throw', 'spell', 'bomb', 'drill', 'glide', 'grapple', 'bridge', 'dash', 'winch', 'stoneWedge', 'reedwing', 'cordAnchor', 'ashwayRites', 'antlerPrybar', 'stoneAdze']) && (value.item === undefined || isItemId(value.item)) && (value.tool === undefined || isTraversalTool(value.tool)) && (value.overdrive === undefined || typeof value.overdrive === 'boolean')
}
