...
export const migrateCampaignRoute = (value: unknown): CampaignRouteState => {
  ...
  return { version: 5, areaOrder: value.version === 4 || value.version === 5 ? [...value.areaOrder!] : value.version === 3 ? [...value.areaOrder!] : [...LEGACY_AREA_ORDER], completedAreas: [...value.completedAreas], unlockedAreas: [...value.unlockedAreas], selectedBiome: value.selectedBiome, rescuedNpcs, companions, lineageEvents: (value.lineageEvents ?? []).map(event => ({ ...event })), legacyRecords: copyLegacyRecords(value.legacyRecords ?? []), alignment: { kami: value.alignment?.kami ?? 0, villagePact: value.alignment?.villagePact ?? 0 }, reputation: { trailfolk: value.reputation?.trailfolk ?? 0, kami: value.reputation?.kami ?? 0 }, cycle: value.cycle ? cloneCampaignCycle(value.cycle) : initialCampaignCycle() }
}
