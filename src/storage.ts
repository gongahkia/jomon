...
import type { Actor, Biome, CampaignRouteState, ConditionState, CourierCalling, CourierMenuEntry, CourierOrigin, CourierSave, DeathMode, EncyclopediaState, Floor, FloorMilestone, GroundItem, Hero, LegacyRecord, LineageEvent, Modal, Point, Prop, Records, RescuedNpc, RunAnalysis, RunFloorMetrics, RunMetricSample, RunState, RunTelemetry, Telegraph, Tile, TransientTerrain, TraversalToolId } from './types'
...
const isFloorBase = (value: unknown): value is Omit<Floor, 'milestones' | 'transientTerrain'> & Partial<Pick<Floor, 'milestones' | 'transientTerrain'>> => isRecord(value) && isInteger(value.index) && oneOf(value.biome, BIOMES) && isNumber(value.seed) && Array.isArray(value.tiles) && value.tiles.length === MAP_WIDTH * MAP_HEIGHT && value.tiles.every(isTile) && Array.isArray(value.actors) && value.actors.every(isActor) && Array.isArray(value.items) && value.items.every(isGroundItem) && Array.isArray(value.props) && value.props.every(isProp) && isPoint(value.start) && isPoint(value.exit) && typeof value.guardianDefeated === 'boolean' && isObjective(value.objective) && (value.telegraphs === undefined || (Array.isArray(value.telegraphs) && value.telegraphs.every(isTelegraph))) && (value.puzzleIds === undefined || (Array.isArray(value.puzzleIds) && value.puzzleIds.every(isString)))
...
const isModal = (value: unknown): value is Modal | undefined => {
  ...
  return value.kind === 'target' && oneOf(value.action, ['throw', 'spell', 'bomb', 'drill', 'glide', 'stoneWedge', 'reedwing', 'cordAnchor', 'ashwayRites']) && (value.item === undefined || isItemId(value.item)) && (value.tool === undefined || isTraversalTool(value.tool)) && (value.overdrive === undefined || typeof value.overdrive === 'boolean')
}
