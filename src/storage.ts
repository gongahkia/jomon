...
import type { Actor, Alignment, AreaArcState, Biome, CampaignRouteState, ClimbLink, ConditionState, CourierCalling, CourierMenuEntry, CourierOrigin, CourierSave, CurseState, DeathMode, DifficultyContext, EncounterKind, EncyclopediaState, Floor, FloorEncounter, FloorEscalation, FloorMilestone, GroundItem, Hero, InteractionTelemetry, LegacyRecord, LineageEvent, Modal, OathState, OptionalContentTelemetry, Point, Prop, Records, RelicId, RescuedNpc, RewardOffer, RunAnalysis, RunFloorMetrics, RunMetricSample, RunState, RunTelemetry, SecretClueChannel, SecretRoom, SecretRoute, ShortcutReturn, SocialContract, SocialReputation, Telegraph, Tile, TransientTerrain, TraversalToolId } from './types'
...
const isRunRecord = (value: unknown): value is RunRecord => isRecord(value) && isNumber(value.version) && isNumber(value.seed) && isFloor(value.floor) && isHero(value.hero) && Array.isArray(value.messages) && value.messages.every(isString) && oneOf(value.status, ['title', 'playing', 'dead', 'victory']) && isModal(value.modal) && isNumber(value.turn) && (value.area === undefined || oneOf(value.area, BIOMES)) && (value.areaFloor === undefined || isNumber(value.areaFloor)) && (value.areaArc === undefined || isAreaArcState(value.areaArc)) && (value.areaOrder === undefined || isAreaOrder(value.areaOrder)) && (value.gateDestination === undefined || oneOf(value.gateDestination, BIOMES)) && (value.shortcutReturn === undefined || isShortcutReturn(value.shortcutReturn)) && (value.rescuedNpcs === undefined || (Array.isArray(value.rescuedNpcs) && value.rescuedNpcs.every(isRescuedNpc))) && (value.lineageEvents === undefined || (Array.isArray(value.lineageEvents) && value.lineageEvents.every(isLineageEvent))) && (value.alignment === undefined || isAlignment(value.alignment)) && (value.reputation === undefined || isSocialReputation(value.reputation)) && (value.encyclopedia === undefined || isEncyclopedia(value.encyclopedia)) && (value.telemetry === undefined || isRunTelemetry(value.telemetry))
