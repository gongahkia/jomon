...
local function isKnowledge(c) return c.features and c.features.knowledge==1 end
local function isEducation(c) return c.features and c.features.education==1 end
local function isBody(c) return c.features and c.features.body==1 end
local function isVisibility(c) return c.features and c.features.visibility==1 end
local function isEquipment(c) return c.features and c.features.equipment==1 end
...
function Campaign.extinct(c)
 local living=0;for _,site in ipairs(Campaign.sites(c)) do if site.world then living=living+W.alive(site.world) end end
...
local function validateRegion(c)
 ...
 for _,notice in ipairs(c.region.notices) do
  ...
  U.integer(notice.tick,'notice tick',0,c.tick);U.integer(notice.siteId,'notice site ID',1,count);U.integer(notice.ordinal,'notice ordinal',0,100000000);U.integer(notice.count,'notice count',1,100000000)
...
function Campaign.validate(c)
 ...
 if c.features.knowledge~=nil then assert(c.features.knowledge==1,'Unsupported campaign knowledge feature version') end
 if c.features.education~=nil then assert(c.features.education==1,'Unsupported campaign education feature version') end
 if c.features.body~=nil then assert(c.features.body==1,'Unsupported campaign body feature version') end
 if c.features.visibility~=nil then assert(c.features.visibility==1,'Unsupported campaign visibility feature version') end
 if c.features.equipment~=nil then assert(c.features.equipment==1,'Unsupported campaign equipment feature version') end
 ...
 assert((c.features.knowledge==1)==(c.knowledge~=nil),'Campaign knowledge feature/state mismatch')
 assert((c.features.education==1)==(c.education~=nil),'Campaign education feature/state mismatch')
 assert((c.features.equipment==1)==(c.equipment~=nil),'Campaign equipment feature/state mismatch')
 ...
 assert(c.features.education==nil or c.features.knowledge==1,'Campaign education requires knowledge')
 assert(c.features.visibility==nil or c.features.body==1,'Campaign visibility requires body')
 assert(c.features.equipment==nil or (c.features.body==1 and c.features.visibility==1),'Equipment requires current body and visibility')
 ...
 if isKnowledge(c) then require('src.knowledge').validateCampaign(c.knowledge,c.tick) end
 if isEducation(c) then require('src.education').validateCampaign(c) end
...
local function attach(world,siteId,nextPerson,knowledge,education,body,visibility,equipment,safeExcavation,psychology,industry,factions,security,environments,relics,environment,seed)
 ...
 if knowledge then require('src.knowledge').attach(worker) end
  if education then require('src.education').attach(worker,seed) end
  if safeExcavation then worker.stress=0;worker.panic=false;worker.lastStressTick=0 end
  if psychology then require('src.psychology').attach(worker,seed) end
 end
 return nextPerson
end
function Campaign.new(source,options)
 ...
 if body then c.features.body=1 end
 if visibility then c.features.visibility=1 end
 if equipment then c.features.equipment=1;c.equipment=require('src.equipment').new() end
 ...
 if security then c.features.security=1;require('src.security').initialise(c) end
...
function Campaign.newRegion(master,options)
 ...
 for _,id in ipairs(bodyOrder(options and options.bodyOrder)) do
  ...
  if id==1 then
   recipe=recipeFrom(home,true);world=G.make(terrainSeed,home.preset,home.mode,home.width,home.height,{layout=home.layout,climate=home.climate,openness=home.openness,biomeScale=home.biomeScale,features=home.features,density=home.density,crew=home.crew,environment=Environments and 'home_planet' or nil})
  else
   ...
   recipe=recipeFrom(moon,false);world=G.makeUnpopulated(terrainSeed,home.mode,moon.width,moon.height,{layout=moon.layout,climate=moon.climate,openness=moon.openness,biomeScale=moon.biomeScale,features=moon.features,density=moon.density,crew=moon.crew,environment=Environments and (id==2 and 'moon_i' or 'moon_ii') or nil})
  end
  ...
  if Environments then bodies[id].environment=Environments.established(id);bodies[id].visited=true end
 ...
 if home.education then assert(home.knowledge,'Education campaigns require knowledge');c.features.education=1;c.education=require('src.education').newCampaign() end
 if home.body then c.features.body=1 end
 if home.visibility then assert(home.body,'Visibility campaigns require body');c.features.visibility=1 end
 if home.equipment then assert(home.body and home.visibility,'Equipment campaigns require current body and visibility');c.features.equipment=1;c.equipment=require('src.equipment').new() end
 ...
 E.create(c,1,'pickaxe',x-2,y);E.create(c,1,'pickaxe',x+2,y)
  E.create(c,1,'rope_coil',x-2,y-1);E.create(c,1,'rope_coil',x+2,y-1);E.create(c,1,'rope_coil',x-2,y-2);E.create(c,1,'rope_coil',x+2,y-2)
  -- Starter tools are part of the initial mineral baseline, rather than a
  -- hidden grant excluded from the ordinary accounting view.
  w.baseline=Metrics.measure(w);w.baseline.mineral=w.baseline.mineral+8
 end
 Campaign.validate(c);return c
end
function Campaign.instantiate(c,siteId)
 assert(isEnvironments(c),'Lazy local worlds require environments')
 local site=Campaign.site(c,siteId);assert(site and site.bodyId>=4,'Only frontier destinations are lazy')
 if site.world then return site.world end
 local body=Campaign.body(c,site.bodyId);assert(body and not body.visited,'Invalid lazy frontier body')
 local G=require('src.generate');local recipe=body.recipe
 local world=G.makeUnpopulated(body.terrainSeed,c.mode,recipe.width,recipe.height,{layout=recipe.layout,climate=recipe.climate,openness=recipe.openness,biomeScale=recipe.biomeScale,features=recipe.features,density=recipe.density,crew=recipe.crew,environment=body.environment.archetype})
...
function Campaign.metrics(c)
 ...
 local measured=Metrics.measure(site.world)
  if isEquipment(c) then measured.equipmentMineral=require('src.equipment').siteMineral(c,site.id);measured.mineral=measured.mineral+measured.equipmentMineral end
  if c.features.logistics==1 then
   ...
   measured.mineral=measured.mineral+reconciliation.cargo.stone+reconciliation.cargo.soil+reconciliation.cargo.metal+2*(reconciliation.cargo.component or 0)
  ...
  if Travel then
   ...
   local mineralExport=account.exports.stone+account.exports.soil+account.exports.metal+2*(account.exports.component or 0)
   local mineralImport=account.imports.stone+account.imports.soil+account.imports.metal+2*(account.imports.component or 0)
   local mineralConsumed=account.consumed.stone+account.consumed.soil+account.consumed.metal+2*(account.consumed.component or 0)
 ...
 totals.mineral=(totals.mineral or 0)+transit.cargo.stone+transit.cargo.soil+transit.cargo.metal+2*(transit.cargo.component or 0)
  if isEquipment(c) then totals.mineral=totals.mineral+require('src.equipment').transitMineral(c) end
 end
...
local function noticesForSite(c,site)
 ...
 if not site.world then return end
...
function Campaign.step(c,commands,clock)
 ...
 for _,site in ipairs(sites) do if site.world then Sim.begin(site.world);assert(site.world.tick==c.tick,'Campaign site tick diverged') end end
 ...
 if site.world then
  ...
  if isEnvironments(c) then require('src.environments').step(c,site) end
 end
 end
