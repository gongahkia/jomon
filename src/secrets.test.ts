...
describe('secret metadata', () => {
  it('serializes deterministic versioned hidden rooms, side pockets, and passages', () => {
    ...
    expect(mine.secretRooms).toEqual(expect.arrayContaining([expect.objectContaining({ version: 1, kind: 'hidden-room', entryCondition: 'sealed-breakwall', discoveryClue: expect.any(String), accessMethod: 'breach', rewardClass: 'supplies', risk: 'dust', safeFallback: true })]))
