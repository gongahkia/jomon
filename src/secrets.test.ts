...
describe('secret metadata', () => {
  it('serializes deterministic versioned hidden rooms, side pockets, and passages', () => {
    const mine = generateAreaFloor(7, 'mine', 1, 3)
    const cliffs = generateAreaFloor(7, 'cliffs', 0, 3)
    expect(mine.secretRooms).toHaveLength(2)
    ...
    expect(mine.secretRoutes).toEqual(expect.arrayContaining([expect.objectContaining({ kind: 'concealed-passage' }), expect.objectContaining({ kind: 'rare-transition', rewardClass: 'shortcut', destination: { biome: 'mine', floor: 3 }, direction: 'one-way', arrival: 'floor-start', returnSemantics: 'no-return' })]))
    ...
    expect(generateAreaFloor(7, 'mine', 1, 3).secretRoutes).toEqual(mine.secretRoutes)
  })
