...
function UI.draw(w,point,sc,app,r,context)
 local function visible(x,y) return not Visibility.enabled(w) or Visibility.currentlyVisible(w,x,y,context) end
 if w.content then
  for _,p in ipairs(w.content.flora) do if p.alive and visible(p.x,p.y) then
  ...
  for _,p in ipairs(w.content.sites) do if p.alive and visible(p.x,p.y) then
  ...
  for _,c in ipairs(w.content.fauna) do if c.alive and visible(c.x,c.y) then
  ...
  if app.view==4 and not Visibility.enabled(w) then
 ...
 for slot=1,w.cols*w.rows do local s=w.structures[slot]
  if s and (s.kind=='charge' or s.kind=='ward') and visible(s.gx*4-2,s.gy*4-2) then
 ...
 for _,fx in ipairs(w.blastEffects or {}) do if visible(fx.x,fx.y) then
