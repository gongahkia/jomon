...
const themeIcon: Record<VotingOption['recipe']['terrain']['theme'], string> = { balanced: '⛳', speedway: '⚡', 'hazard-run': '⚠', 'ice-rink': '❄', quarry: '⛏', drift: '↻', bloom: '✽', pulse: '⌁', carnival: '★', marsh: '▤', zephyr: '〰' };
const themeDescriptor: Record<VotingOption['recipe']['terrain']['theme'], string> = { balanced: 'steady green', speedway: 'speed lanes', 'hazard-run': 'moving traps', 'ice-rink': 'long slides', quarry: 'hard climbs', drift: 'paired sinkholes', bloom: 'thorn knockback', pulse: 'launch fields', carnival: 'spring banks', marsh: 'cushion turf', zephyr: 'gust lanes' };
...
const renderVoteOverlay = (view: ViewModel) => {
  ...
  const packageCard = (option: VotingOption, index: number) => {
    ...
    return `<article class="vote-card theme-${terrain.theme}" data-vote-option="${option.id}" role="button" tabindex="0" aria-label="vote for ${escapeHtml(option.label)}"><header><span class="package-number">${index + 1}</span><div><p class="card-kicker" title="${escapeHtml(terrain.theme)}">${themeIcon[terrain.theme]} ${themeDescriptor[terrain.theme]}</p><h2>${escapeHtml(option.label)}</h2></div><div class="vote-pills"><strong class="vote-count">${votesFor(option.id)}<small>votes</small></strong>${voterPills}</div></header>
      <div class="course-glyphs" aria-label="course configuration"><span title="course dimensions and size profile">▣ ${terrain.width}×${terrain.height} · ${terrain.sizeProfile}</span><span title="route archetype">⌁ ${terrain.archetype}</span><span title="route length">🧭 ${Math.round(terrain.routeLength * 100)}%</span><span title="bendiness">↪ ${Math.round(terrain.bendiness * 100)}%</span><span title="lane width">↔ ${terrain.laneWidth}</span><span title="ramps">⛰ ${Math.round(terrain.elevation * 100)}%/${terrain.maxElevation}</span><span title="walls and bumpers">🧱 ${terrain.wallCount} · ◇ ${terrain.bumperCount}</span><span title="sweepers and gates">⚠ ${terrain.sweeperCount}+${terrain.gateCount}</span><span title="air hazards: updrafts and low bars">⌃ ${terrain.updraftCount} · ⊓ ${terrain.lowBarCount}</span><span title="gust lanes">〰 ${terrain.gustCount}</span><span title="spring and cushion turf">⌃ ${Math.round(terrain.springRate * 100)}% · ▤ ${Math.round(terrain.cushionRate * 100)}%</span><span title="air rings">◯ ${terrain.airRingCount}</span><span title="portal pairs">◉ ${terrain.portalPairs}</span><span title="item pads">✚ ${terrain.recoveryPads} · ✹ ${terrain.chaosPads}</span>${terrain.theme === 'drift' ? `<span title="paired sinkholes">↻ ${terrain.sinkholePairs}</span>` : terrain.theme === 'bloom' ? `<span title="thorn zones">✽ ${terrain.thornCount}</span>` : terrain.theme === 'pulse' ? `<span title="pulse fields">⌁ ${terrain.pulseCount}</span>` : terrain.theme === 'carnival' ? '<span title="carnival spring banks">★ spring banks</span>' : terrain.theme === 'marsh' ? '<span title="marsh cushion turf">▤ cushion turf</span>' : terrain.theme === 'zephyr' ? '<span title="zephyr gust lanes">〰 gust lanes</span>' : ''}</div>
