...
const boonIcon: Record<string, string> = {
  'heavy ball': '●', 'ice skates': '⛸', 'extra charge': '✚', 'bank shot': '↩', 'hazard shield': '⬡', 'chaos magnet': '🧲', 'portal savvy': '◉', 'second wind': '↯', scavenger: '▣', 'aerial ace': '⌒', 'cup reader': '◌', gadgeteer: '⚒',
};
const powerUpIcon: Record<string, string> = { turbo: '↯', shield: '⬡', bomb: '✹', freeze: '❄', swap: '⇄', 'two putts': '2P', heavy: '●', bouncy: '◌', ghost: '◐', magnet: '🧲', ice: '❄', portal: '◉', glider: '⌒', sticky: '▣', orbit: '◎', 'cup magnet': '⊙', slipstream: '➳', 'rebound rig': '↩', 'phase shift': '⇤', sandbag: '▰', 'rescue drone': '✈', airhorn: '📣', 'popper pad': '↑', 'snare patch': '⌁', 'blast mine': '✹', 'slick patch': '≋', 'sky spring': '⌃' };
...
const renderTransitionControls = (state: GameState) => `<div class="turn"><strong>next hole</strong><b>${state.hole}/${state.config.holeCount}</b><small class="phase">course rebuilding</small></div><p class="control-status">${renderStatus({ state, shotInFlight: false })}</p>`;
