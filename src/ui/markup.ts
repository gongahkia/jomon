...
const renderMatchHud = (view: ViewModel) => {
  ...
  return `<section class="match-hud" aria-label="match heads-up display"><section class="hud-course"><p>COURSE</p><strong>${themeIcon[state.course.theme]} ${escapeHtml(themeDescriptor[state.course.theme])}</strong><small>hole ${state.hole} · par ${state.course.score.estimatedStrokes} · ${state.course.width}×${state.course.height}</small></section><section class="hud-timer"><p>ON THE TEE</p><strong>${escapeHtml(player.name)}</strong><b id="hud-turn-timer">${state.turn.secondsLeft.toFixed(0)}<small>s</small></b></section><section class="hud-leaderboard" aria-label="leaderboard"><header><span>LEADERBOARD</span><small>total</small></header><ol>${leaderboard}</ol></section><section class="hud-strength" aria-label="${shotKind} strength"><div class="hud-strength-heading"><span id="hud-shot-label">${shotKind === 'chip' ? 'CHIP' : 'PUTT'} STRENGTH</span><b id="hud-strength-value">${aim.power.toFixed(1)}</b></div><div id="hud-strength-meter" class="hud-strength-meter" style="--strength:${strength}%"><i></i></div><div class="hud-shot-mode" role="group" aria-label="shot type"><button data-shot-kind="putt" class="${shotKind === 'putt' ? 'selected' : ''}" aria-pressed="${shotKind === 'putt'}" ${disabled || player.forcedChip ? 'disabled' : ''}>◌ putt</button><button data-shot-kind="chip" class="${shotKind === 'chip' ? 'selected' : ''}" aria-pressed="${shotKind === 'chip'}" ${disabled || player.forcedChip ? 'disabled' : ''}>⌒ chip</button></div></section><section class="pull-control ${disabled ? 'disabled' : ''}" aria-label="pull-back shot control"><p>DRAG BACK · RELEASE TO STRIKE</p><div class="pull-pad"><span class="pull-pad-mark">course aim</span><button id="pull-ball" type="button" aria-label="drag the ball back to aim and set strength" aria-describedby="pull-instruction" ${disabled ? 'disabled' : ''} style="--pull-color:${player.color}"></button></div><small id="pull-instruction">pull farther for more strength</small></section></section>`;
};
