...
const renderDieOverlay = (view: ViewModel) => {
  ...
  const slotSymbol = (face: typeof die.faces[number]) => `<span class="slot-symbol" aria-hidden="true">${themeIcon[face.recipe.terrain.theme]}</span><span class="slot-symbol-label">${escapeHtml(themeDescriptor[face.recipe.terrain.theme])}</span>`;
