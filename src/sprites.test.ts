...
import { actorSprite, generatedSpriteAssetsAvailable, itemSprite, spriteSheetSpecs } from './sprites'
...
describe('sprite registry', () => {
  ...
  it('retains atlas metadata without generated images', () => {
    expect(generatedSpriteAssetsAvailable).toBe(false)
    expect(spriteSheetSpecs.every(sheet => sheet.url === undefined)).toBe(true)
  })
})
