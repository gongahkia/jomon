...
const render = () => {
  ...
  app.innerHTML = `
    <section class="topbar"><div><p class="eyebrow">ASCII ROGUELIKE MINI GOLF</p><h1>GOLF <em>WITH YOUR</em> ENEMIES</h1></div><div class="title-actions"><button data-open-overlay="help">? help</button><button data-open-overlay="settings">F1 settings</button><div class="hole">HOLE <b>${state.hole}</b> / 9<br><small>${escape(state.course.seed)}</small></div></div></section>
    <section class="layout">
      ...
      <section class="board panel"><div id="status">${renderStatus()}</div><div class="course-stage"><canvas id="course" aria-label="isometric ASCII golf course"></canvas><div id="callouts" class="callouts" aria-live="polite">${calloutMarkup()}</div></div><div id="controls" class="controls"></div></section>
...
const renderInspector = () => {
  ...
  if (state.status === 'draft') return `<h2>upgrade draft</h2><p>each player keeps one modifier for the rest of the campaign.</p><div class="upgrades">${currentUpgradeChoices(state).map((upgrade) => `<button data-upgrade="${upgrade}">${upgrade}</button>`).join('')}</div>${renderLedger()}`;
  return `<h2>shot ledger</h2>${renderLedger()}<h3>ASCII legend</h3><p class="legend">T tee · O cup · # wall · : sand · ~ ice<br>&gt; booster · = conveyor</p>`;
};
