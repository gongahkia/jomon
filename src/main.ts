...
const setState = (next: GameState) => {
  ...
  state = next;
  if (state.status === 'preview' && previousStatus !== 'preview') drawer = 'intel';
...
const chooseAim = (event: PointerEvent) => {
  ...
  aim = renderer.aimFromPointer(event, state.course, current().ball, state.rotation);
...
const renderInspector = () => {
  ...
  if (state.status === 'draft') return `<p>each player keeps one modifier for the rest of the campaign.</p><div class="upgrades">${currentUpgradeChoices(state).map((upgrade) => `<button data-upgrade="${upgrade}">${upgrade}</button>`).join('')}</div>${renderLedger()}`;
  return `${renderLedger()}<h3>course legend</h3><p class="legend">quarry stone · sand drift · ice sheet<br>red chevrons guide the route · violet diamonds hold chaos items</p>`;
};
