...
export interface AutoplayTraceRecord { version: typeof AUTOPLAY_TRACE_VERSION; sequence: number; episode: AutoplayTraceEpisode; turn: number; replay: AutoplayReplayMetadata; observation: AutoplayTraceObservation; legalCandidates: AutoplayCandidate[]; chosen: { command: string; reason: string }; outcome: { events: string[]; nextFingerprint: string; status: RunState['status'] }; resourceDelta: AutoplayTraceResourceDelta; previousHash: string | null; hash: string }
...
const traceRecordValue = (record: Omit<AutoplayTraceRecord, 'hash'>): Omit<AutoplayTraceRecord, 'hash'> => ({ version: record.version, sequence: record.sequence, episode: record.episode, turn: record.turn, replay: record.replay, observation: record.observation, legalCandidates: record.legalCandidates, chosen: record.chosen, outcome: record.outcome, resourceDelta: record.resourceDelta, previousHash: record.previousHash })
