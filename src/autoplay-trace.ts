...
import { MAP_WIDTH, type AutoplayCandidate, type AutoplayMode, type AutoplayOptionalAssessment, type AutoplayReplayMetadata, type AutoplayResourceAssessment, type AutoplayToolAssessment, type RunState, type TileKind } from './types'
...
export interface AutoplayTraceRecord { version: typeof AUTOPLAY_TRACE_VERSION; sequence: number; episode: AutoplayTraceEpisode; turn: number; replay: AutoplayReplayMetadata; observation: AutoplayTraceObservation; features: PolicyFeatureVector; legalCandidates: AutoplayCandidate[]; resourceDiagnostics?: AutoplayResourceAssessment[]; toolDiagnostics?: AutoplayToolAssessment[]; optionalDiagnostics?: AutoplayOptionalAssessment[]; chosen: { command: string; reason: string }; outcome: { events: string[]; nextFingerprint: string; status: RunState['status'] }; resourceDelta: AutoplayTraceResourceDelta; previousHash: string | null; hash: string }
export interface AutoplayTraceTerminal { outcome: 'complete' | 'dead' | 'stalled' | 'turn-limit' | 'unsupported' | 'error'; reason: string; turns: number; campaignComplete: boolean; finalFingerprint: string }
...
const traceRecordValue = (record: Omit<AutoplayTraceRecord, 'hash'>): Omit<AutoplayTraceRecord, 'hash'> => ({ version: record.version, sequence: record.sequence, episode: record.episode, turn: record.turn, replay: record.replay, observation: record.observation, features: record.features, legalCandidates: record.legalCandidates, ...(record.resourceDiagnostics?.length ? { resourceDiagnostics: record.resourceDiagnostics } : {}), ...(record.toolDiagnostics?.length ? { toolDiagnostics: record.toolDiagnostics } : {}), ...(record.optionalDiagnostics?.length ? { optionalDiagnostics: record.optionalDiagnostics } : {}), chosen: record.chosen, outcome: record.outcome, resourceDelta: record.resourceDelta, previousHash: record.previousHash })
