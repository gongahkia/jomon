...
import { MAP_WIDTH, type AutoplayCandidate, type AutoplayMode, type AutoplayReplayMetadata, type AutoplayResourceAssessment, type RunState, type TileKind } from './types'
...
const traceRecordValue = (record: Omit<AutoplayTraceRecord, 'hash'>): Omit<AutoplayTraceRecord, 'hash'> => ({ version: record.version, sequence: record.sequence, episode: record.episode, turn: record.turn, replay: record.replay, observation: record.observation, features: record.features, legalCandidates: record.legalCandidates, ...(record.resourceDiagnostics?.length ? { resourceDiagnostics: record.resourceDiagnostics } : {}), chosen: record.chosen, outcome: record.outcome, resourceDelta: record.resourceDelta, previousHash: record.previousHash })
