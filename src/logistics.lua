...
local function physicalCargo(c,craftId)
 local n=c.features and c.features.equipment==1 and require('src.equipment').craftCount(c,craftId) or 0
 if c.features and c.features.relics==1 then n=n+require('src.relics').craftCount(c,craftId) end
 return n
end
...
function L.validate(c)
 ...
 for i=1,craftCount do
  ...
  assert(packed+physicalCargo(c,record.id)<=record.capacity,'Craft physical cargo exceeds capacity')
 ...
 for i=1,manifestCount do
  ...
  assert(planned+physicalCargo(c,vehicle.id)<=vehicle.capacity,'Manifest plus physical cargo exceeds capacity')
...
local function preparePlan(c,command)
 ...
 assert(cargoTotal(cargo)+physicalCargo(c,vehicle.id)<=vehicle.capacity,'Cargo exceeds craft capacity')
...
function L.loadDeliver(c,siteId,job,a)
 ...
 if carry.n>job.logistics.remaining or current+carry.n>(m.cargo[carry.kind] or 0) or cargoTotal(vehicle.cargo)+carry.n+physicalCargo(c,vehicle.id)>vehicle.capacity then return false,'Craft cargo target changed' end
