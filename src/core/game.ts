...
const cloneState = (state: GameState): GameState => ({ ...state, players: state.players.map((player) => ({ ...player, ball: { ...player.ball }, upgrades: [...player.upgrades] })), turn: { ...state.turn }, messages: [...state.messages] });
...
export const botMove = (state: GameState): ShotCommand | undefined => {
  ...
  return chooseBotDecision(state.course, player, state.players).shot;
};
