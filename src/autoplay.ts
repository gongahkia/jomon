...
export const autoplayStateFingerprint = (state: RunState): string => {
  ...
  const companions = (state.companions ?? []).map(companion => `${companion.id}:${companion.rosterStatus}:${companion.controlMode}:${companion.injury}:${Object.entries(companion.abilityState.cooldowns).sort(([left], [right]) => left.localeCompare(right)).map(([id, turns]) => `${id}${turns}`).join(',')}`).sort().join('|')
