...
const validProjectionValue = (value: unknown): value is ManagementSidebarFactValue => {
  ...
  if (value.kind === 'causal-command') return hasOnlyKeys(value, ['kind', 'sequence', 'commandKind']) && safeInteger(value.sequence) && oneOf(['initial-courier-selected', 'tavern-courier-switched', 'vessel-station-readout-recorded', 'courier-loss-resolved', 'time-bearing-action', 'deck-moved', 'durable-jomon-growth', 'delegation-offered', 'delegation-interrupted'] as const, value.commandKind)
