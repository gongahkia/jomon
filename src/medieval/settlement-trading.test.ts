...
describe('local settlement freight handoff', () => {
  ...
  it('delivers the accepted material burden at the cargo hold through one replayed zero-time receipt', () => {
    ...
    expect(delivered.state.markets.markets.find(market => market.id === 'market:settlement-location:hearthford-mill-quay')?.commodityStates[0]).toEqual({ commodityId: 'commodity:ironwork', stock: 'none', demand: 'high', price: 'high' })
