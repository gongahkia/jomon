...
const validWorld = (value: unknown): value is FoundationWorld => {
  if (!record(value) || value.version !== 13 || value.status !== 'active' || !record(value.manifest) || !record(value.manifest.creation) || !record(value.state)) return false
  try {
...
export const createFidelityPlanForVerifiedWorld = (world: FoundationWorld, activeCourierId: string): FidelityPlan => {
  if (!record(world) || world.version !== 13 || world.status !== 'active' || !record(world.manifest) || !record(world.manifest.creation) || !record(world.state)
