...
export const createTerminalCommandHelpModel = (preferences: TerminalControlPreferences): TerminalCommandHelpModel => {
  ...
  const entries = TERMINAL_CONTROL_DEFINITIONS.map(definition => {
    ...
    const state = definition.operationalState === 'movement-available'
      ...
      : definition.operationalState === 'opens-contextual-prompt'
        ? 'Opens the source-backed contextual prompt. Every physical station has a bounded zero-time readout; the tavern task ledger alone can switch the active courier.'
  ...
  return { entries, accessibilitySummary: `Command help. ${entries.length} remappable world controls. The static Jomon deck is visible; movement is local and collision-checked. The contextual control opens the source-backed zero-time readout at the current physical station; only the task ledger also provides courier switching.` }
}
