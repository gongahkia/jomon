import type { AutoplayMode, AutoplayPolicy, KeyBindingId } from './types'

export interface GameSettings { version: 1; reducedFlash: boolean; autoplayMode: AutoplayMode; autoplayPolicy: AutoplayPolicy; bindings: Partial<Record<KeyBindingId, string>> }
...
export const normalizeSettings = (value: unknown): GameSettings => {
  ...
  const autoplayPolicy: AutoplayPolicy = source.autoplayPolicy === 'clear' || source.autoplayPolicy === 'explore' || source.autoplayPolicy === 'legacy' ? source.autoplayPolicy : 'survival'
  const candidate: GameSettings = { version: 1, reducedFlash: source.reducedFlash === true, autoplayMode, autoplayPolicy, bindings }
