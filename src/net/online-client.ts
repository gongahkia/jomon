import type { ClientMessage, RoomClock, RoomSnapshot, ServerMessage } from './protocol';

export interface OnlineClientHandlers {
  onRoom(room: RoomSnapshot): void;
  onTick(clock: RoomClock): void;
  ...
  connect(url: string, onOpen?: () => void) {
    ...
    socket.addEventListener('message', (event) => {
      try {
        ...
        if (message.type === 'room-tick') this.handlers.onTick(message.clock);
