...
describe('persistent destination partitions', () => {
  ...
  it('migrates v3 saves deterministically without simulating their historical Route Reckoning', () => {
    ...
    expect(first).toMatchObject({ version: 6, routeReckoning: 900, destinationWorld: { partitions: { 'destination:nerida': { condition: 'pump-watch', lastProcessedRouteReckoning: 900, scheduledDevelopments: [{ id: 'development:nerida:pump-cavitation', dueAtRouteReckoning: 1_380 }] } } }, institutionWorld: { operations: [], causalEvents: [] } })
