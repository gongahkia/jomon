...
export const initialCampaignRoute = (seed?: number): CampaignRouteState => {
  ...
  return { version: 5, areaOrder, completedAreas: [], unlockedAreas: [areaOrder[0]], selectedBiome: areaOrder[0], rescuedNpcs: [], companions: [], lineageEvents: [], legacyRecords: [], alignment: { kami: 0, villagePact: 0 }, reputation: { trailfolk: 0, kami: 0 }, cycle: initialCampaignCycle() }
}
export const completeCampaignArea = (state: CampaignRouteState, completed: Biome): CampaignRouteState => ({ ...state, areaOrder: [...state.areaOrder], completedAreas: state.completedAreas.includes(completed) ? [...state.completedAreas] : [...state.completedAreas, completed], unlockedAreas: [...state.unlockedAreas], selectedBiome: completed, rescuedNpcs: [...state.rescuedNpcs], companions: cloneCompanions(state.companions, state.rescuedNpcs), lineageEvents: [...state.lineageEvents], legacyRecords: [...state.legacyRecords], alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: cloneCampaignCycle(state.cycle) })
export const unlockCampaignArea = (state: CampaignRouteState, biome: Biome): CampaignRouteState => ({ ...state, areaOrder: [...state.areaOrder], completedAreas: [...state.completedAreas], unlockedAreas: state.unlockedAreas.includes(biome) ? [...state.unlockedAreas] : [...state.unlockedAreas, biome], selectedBiome: biome, rescuedNpcs: [...state.rescuedNpcs], companions: cloneCompanions(state.companions, state.rescuedNpcs), lineageEvents: [...state.lineageEvents], legacyRecords: [...state.legacyRecords], alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: cloneCampaignCycle(state.cycle) })
export const recordCampaignSacrifice = (state: CampaignRouteState, event: LineageEvent): CampaignRouteState => {
  ...
  return { ...state, rescuedNpcs, companions: cloneCompanions(state.companions.map(companion => loseCompanionForRescue(companion, event.npcId)), rescuedNpcs), lineageEvents: state.lineageEvents.some(existing => existing.id === event.id) ? [...state.lineageEvents] : [...state.lineageEvents, event].slice(-12), alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: cloneCampaignCycle(state.cycle) }
}
export const appendLegacyRecord = (state: CampaignRouteState, record: LegacyRecord): CampaignRouteState => ({ ...state, companions: cloneCompanions(state.companions, state.rescuedNpcs), legacyRecords: [...state.legacyRecords, { ...record }].slice(-12), alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: cloneCampaignCycle(state.cycle) })
export const addAlignment = (state: CampaignRouteState, alignment: Alignment): CampaignRouteState => ({ ...state, companions: cloneCompanions(state.companions, state.rescuedNpcs), alignment: { ...state.alignment, [alignment]: state.alignment[alignment] + 1 }, cycle: cloneCampaignCycle(state.cycle) })
