...
export const continueCampaignRoute = (state: CampaignRouteState): CampaignRouteState => {
  ...
  if (!campaignContinuationPending(cycle)) return { ...state, areaOrder: [...state.areaOrder], completedAreas: [...state.completedAreas], unlockedAreas: [...state.unlockedAreas], rescuedNpcs: state.rescuedNpcs.map(npc => ({ ...npc })), companions: cloneCompanions(state.companions, state.rescuedNpcs), carryoverDiagnostics: cloneCarryoverDiagnostics(state.carryoverDiagnostics), lineageEvents: state.lineageEvents.map(event => ({ ...event })), legacyRecords: state.legacyRecords.map(record => ({ ...record })), alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle }
  const selectedBiome = state.areaOrder[0]
  if (!selectedBiome) throw new Error('cannot continue campaign: missing area order')
  return { ...state, areaOrder: [...state.areaOrder], completedAreas: [], unlockedAreas: [selectedBiome], selectedBiome, rescuedNpcs: state.rescuedNpcs.map(npc => ({ ...npc })), companions: cloneCompanions(state.companions, state.rescuedNpcs), carryoverDiagnostics: cloneCarryoverDiagnostics(state.carryoverDiagnostics), lineageEvents: state.lineageEvents.map(event => ({ ...event })), legacyRecords: state.legacyRecords.map(record => ({ ...record })), alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: advanceCampaignTier(cycle) }
}
...
export const initialCampaignRoute = (seed?: number): CampaignRouteState => {
  ...
  return { version: 5, areaOrder, completedAreas: [], unlockedAreas: [areaOrder[0]], selectedBiome: areaOrder[0], rescuedNpcs: [], companions: [], carryoverDiagnostics: [], lineageEvents: [], legacyRecords: [], alignment: { kami: 0, villagePact: 0 }, reputation: { trailfolk: 0, kami: 0 }, cycle: initialCampaignCycle() }
}
export const completeCampaignArea = (state: CampaignRouteState, completed: Biome): CampaignRouteState => ({ ...state, areaOrder: [...state.areaOrder], completedAreas: state.completedAreas.includes(completed) ? [...state.completedAreas] : [...state.completedAreas, completed], unlockedAreas: [...state.unlockedAreas], selectedBiome: completed, rescuedNpcs: [...state.rescuedNpcs], companions: cloneCompanions(state.companions, state.rescuedNpcs), carryoverDiagnostics: cloneCarryoverDiagnostics(state.carryoverDiagnostics), lineageEvents: [...state.lineageEvents], legacyRecords: [...state.legacyRecords], alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: cloneCampaignCycle(state.cycle) })
export const unlockCampaignArea = (state: CampaignRouteState, biome: Biome): CampaignRouteState => ({ ...state, areaOrder: [...state.areaOrder], completedAreas: [...state.completedAreas], unlockedAreas: state.unlockedAreas.includes(biome) ? [...state.unlockedAreas] : [...state.unlockedAreas, biome], selectedBiome: biome, rescuedNpcs: [...state.rescuedNpcs], companions: cloneCompanions(state.companions, state.rescuedNpcs), carryoverDiagnostics: cloneCarryoverDiagnostics(state.carryoverDiagnostics), lineageEvents: [...state.lineageEvents], legacyRecords: [...state.legacyRecords], alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: cloneCampaignCycle(state.cycle) })
export const recordCampaignSacrifice = (state: CampaignRouteState, event: LineageEvent): CampaignRouteState => {
  ...
  return { ...state, rescuedNpcs, companions: cloneCompanions(state.companions.map(companion => loseCompanionForRescue(companion, event.npcId)), rescuedNpcs), carryoverDiagnostics: cloneCarryoverDiagnostics(state.carryoverDiagnostics), lineageEvents: state.lineageEvents.some(existing => existing.id === event.id) ? [...state.lineageEvents] : [...state.lineageEvents, event].slice(-12), alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: cloneCampaignCycle(state.cycle) }
}
export const appendLegacyRecord = (state: CampaignRouteState, record: LegacyRecord): CampaignRouteState => ({ ...state, companions: cloneCompanions(state.companions, state.rescuedNpcs), carryoverDiagnostics: cloneCarryoverDiagnostics(state.carryoverDiagnostics), legacyRecords: [...state.legacyRecords, { ...record }].slice(-12), alignment: { ...state.alignment }, reputation: { trailfolk: state.reputation?.trailfolk ?? 0, kami: state.reputation?.kami ?? 0 }, cycle: cloneCampaignCycle(state.cycle) })
export const addAlignment = (state: CampaignRouteState, alignment: Alignment): CampaignRouteState => ({ ...state, companions: cloneCompanions(state.companions, state.rescuedNpcs), carryoverDiagnostics: cloneCarryoverDiagnostics(state.carryoverDiagnostics), alignment: { ...state.alignment, [alignment]: state.alignment[alignment] + 1 }, cycle: cloneCampaignCycle(state.cycle) })
