...
const affectedEntityIds = (references: ManifestReferences): string[] => [references.contractId, references.packageId, references.courierId, references.routeCacheId, references.siteId, references.routeId, references.transitId, references.institutionId, references.actorId, references.causalEventId, references.deliveryRunId, references.itemId, references.encounterId].filter((value): value is string => Boolean(value))
...
export const appendGeneralManifest = (galaxy: GalaxyState, input: ManifestEntryInput): number => {
  ...
  const entry: GeneralManifestEvent = {
    ...
    ...(input.deliveryRunId ? { deliveryRunId: input.deliveryRunId } : {}),
...
export const normalizeGeneralManifest = (galaxy: GalaxyState): void => {
  ...
  for (const [index, raw] of legacyEntries.entries()) {
    ...
    const references: ManifestReferences = { contractId: raw.contractId, packageId: raw.packageId, courierId: raw.courierId, routeCacheId: raw.routeCacheId, siteId: raw.siteId, routeId: raw.routeId, transitId: raw.transitId, institutionId: raw.institutionId, actorId: raw.actorId, causalEventId: raw.causalEventId, deliveryRunId: raw.deliveryRunId, itemId: raw.itemId, encounterId: raw.encounterId }
    const entry: GeneralManifestEvent = {
      ...raw,
      version: 2,
