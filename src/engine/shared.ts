...
export type GameEventType = 'move' | 'traverse' | 'encounter' | 'hit' | 'hurt' | 'pickup' | 'spell' | 'boom' | 'danger' | 'menu' | 'level' | 'rope' | 'suspend' | 'death' | 'win' | 'floor' | 'areaComplete' | 'connectorComplete' | 'routeDeparture' | 'routeCache' | 'gateResolved' | 'rescue' | 'terrain' | 'companion'
...
export const eventLabel = (value: GameEvent): string => value.type === 'terrain' ? `terrain:${value.id ?? 'unknown'}:${value.reason ?? 'unknown'}` : value.type === 'companion' ? `companion:${value.id ?? 'unknown'}:${value.reason ?? 'unknown'}` : value.type
