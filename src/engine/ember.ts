...
export const castEmber = (state: RunState, point: Point): void => {
  const tile = getTile(state.floor, point.x, point.y)
  if (!tile) return
  const target = actorAt(state.floor, point.x, point.y)
  if (target?.hostile) {
    target.health -= modifyIncomingDamage(target, 8 + state.hero.stats.intellect)
  ...
  if (reactions.some(reaction => reaction.reaction === 'ignited-gas' || reaction.reaction === 'detonated-volatile')) {
    explode(state, point.x, point.y, 6, ['fire'], 'the ignition')
    return
  }
