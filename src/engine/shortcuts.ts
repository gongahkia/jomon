...
export const takeSecretShortcut = (state: RunState): ActionResult | undefined => {
  if (state.shortcutReturn && at(state.hero, state.floor.start)) {
    ...
    if (!arrival || !isPassable(returning.floor, returning.arrival.x, returning.arrival.y)) { log(state, 'The return link has no safe landing.'); return [] }
  ...
  if (!safeLanding(state, route) || !route.destination || route.arrival !== 'floor-start' || !route.direction || !route.returnSemantics) { log(state, 'This shortcut cannot leave its biome or bypass an unapproved floor.'); return [] }
  ...
  if (!isPassable(destination, destination.start.x, destination.start.y)) { log(state, 'The shortcut landing is unsafe.'); return [] }
