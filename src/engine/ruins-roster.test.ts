...
describe('Ruins enemy roster', () => {
  ...
  it('raises wards, arms darts, seals doors, and marks targets', () => {
    ...
    const formation = createRun()
    const leader = createEnemy({ id: 'sentinel', kind: 'sentinel', name: 'Sentinel', x: 5, y: 1, speed: 0, energy: 0, encounter: { id: 'tactical:test', archetype: 'objectiveDefense', leader: true, answer: 'draw defenders away' } })
    const support = createEnemy({ id: 'wardacolyte', kind: 'wardacolyte', name: 'Ward Acolyte', ai: 'ranged', x: 4, y: 1, speed: 100, energy: 0, encounter: { id: 'tactical:test', archetype: 'objectiveDefense', leader: false, answer: 'draw defenders away' } })
    formation.floor.actors = [leader, support]
    advance(formation, [])
    expect(leader.conditions).toContainEqual({ kind: 'shielded', duration: 2, potency: 2 })
    expect(support.conditions).not.toContainEqual({ kind: 'shielded', duration: 2, potency: 2 })
