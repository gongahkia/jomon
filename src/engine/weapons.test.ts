...
describe('weapon tactical profiles', () => {
  ...
  it('gives Tidebound couriers a working Tide Spear profile', () => {
    const state = createRun({ hero: createHero({ equipment: { mainHand: 'tideSpear' } }) })
    const distant = createEnemy({ x: 3, y: 1, defense: 0 })
    state.floor.actors = [distant]
    moveHero(state, 'e')
    expect(distant.health).toBeLessThan(distant.maxHealth)
    expect(state.hero.cooldowns?.tideSpear).toBe(1)
  })
})
