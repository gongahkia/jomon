import { floorPoint, type Actor, type GuardianPhase, type RunState, type TileKind } from '../types'
...
export const advanceGuardianPhase = (state: RunState, guardian: Actor): GuardianTransition | undefined => {
  ...
  const candidates = guardian.kind === 'foreman' && next === 'pressure' && (guardianTile?.kind === 'floor' || guardianTile?.kind === 'rail')
    ...
    : state.floor.tiles.flatMap((current, index) => current.kind === 'floor' ? [floorPoint(state.floor, index)] : []).filter(point => !(point.x === state.hero.x && point.y === state.hero.y) && !actorAt(state.floor, point.x, point.y))
