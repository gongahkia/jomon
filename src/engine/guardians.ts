...
export const advanceGuardianPhase = (state: RunState, guardian: Actor): GuardianTransition | undefined => {
  ...
  const tile: TileKind = next === 'pressure' ? 'gas' : 'fireVent'
  const candidates = state.floor.tiles.flatMap((current, index) => current.kind === 'floor' ? [{ x: index % 48, y: Math.floor(index / 48) }] : []).filter(point => !(point.x === state.hero.x && point.y === state.hero.y) && !actorAt(state.floor, point.x, point.y))
