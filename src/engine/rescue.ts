...
export const recordRescue = (state: RunState, actor: Actor | undefined): RescuedNpc => {
  ...
  const npc: RescuedNpc = { id: `rescue:${biome}:${state.floor.index}:${actor?.id ?? `${state.hero.x},${state.hero.y}`}`, name: actor?.name ?? 'Lost Scout', biome, floor: state.areaFloor ?? state.floor.index % 4 }
