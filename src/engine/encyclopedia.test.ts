...
describe('encyclopedia', () => {
  it('records seen enemies, telegraphs, tags, gates, and legacy records', () => {
    ...
    state.floor.actors = [createEnemy({ id: 'fusewarden-1', kind: 'fusewarden', name: 'Fuse Warden', x: 3, y: 1 })]
    ...
    expect(encyclopediaEntries(state, 'enemies')).toContain('Fuse Warden — mine, telegraph, cover, explosive')
    ...
    expect(encyclopediaEntries(state, 'gates')[0]).toContain('mine → wilds')
    expect(encyclopediaEntries(state, 'legacy')).toContain('Ari: defeated, mine 1, active')
  })
