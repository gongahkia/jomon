...
def _torch_transformer_classes() -> tuple[Any, Any, Any]:
    ...
    class MahjongStateTransformerEncoder(nn.Module):
        ...
        def _embedded_tokens(self, state_values: Any) -> tuple[Any, bool]:
    ...
    class DiscardTransformerTensorDataset(dataset_base):
    ...
    globals()["MahjongStateTransformerEncoder"] = MahjongStateTransformerEncoder
    globals()["DiscardTransformerPolicy"] = DiscardTransformerPolicy
    globals()["DiscardTransformerTensorDataset"] = DiscardTransformerTensorDataset
    return MahjongStateTransformerEncoder, DiscardTransformerPolicy, DiscardTransformerTensorDataset
...
def __getattr__(name: str) -> Any:
    if name in {
        ...
        encoder, policy, dataset = _torch_transformer_classes()
        return {
            "MahjongStateTransformerEncoder": encoder,
            "DiscardTransformerPolicy": policy,
            "DiscardTransformerTensorDataset": dataset,
        }[name]
    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for state, legal_mask, target in train_loader:
            ...
            loss = functional.cross_entropy(_policy_logits(model(state, legal_mask)), target)
            loss.backward()
            optimizer.step()
        record_epoch(epoch)
...
def transformer_state_tensor(example: DiscardExample) -> Any:
    torch = require_torch()
...
def transformer_legal_mask(example: DiscardExample) -> Any:
    torch = require_torch()
...
def _batched_state_values(state_values: Any) -> tuple[Any, bool]:
...
def _batched_legal_mask(legal_mask: Any, *, batch_size: int) -> Any:
