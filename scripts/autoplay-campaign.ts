...
const timedOutRun = (entry: typeof entries[number], profile: typeof CAMPAIGN_AUTOPLAY_PROFILES[number]): CampaignAutoplayRun => {
  ...
  return { seed: entry.seed, profile: profile.id, mode: profile.mode, policy: profile.policy, heuristicProfile: autoplayHeuristicProfileRef(heuristicProfile), areaOrder, campaignComplete: false, outcome: 'turn-limit', turns: 0, finalBiome, floor: 1, completedAreas: [], failure: { outcome: 'turn-limit', finalBiome, floor: 1, completedAreas: [], replay: autoplayReplayMetadata(initial), reason: `worker wall-time limit (${workerTimeout}ms)`, trace: [] } }
}
const run = (entry: typeof entries[number], profile: typeof CAMPAIGN_AUTOPLAY_PROFILES[number]): Promise<CampaignAutoplayRun> => new Promise((resolveRun, reject) => {
  const child = spawn(resolve('node_modules/.bin/tsx'), [resolve('scripts/autoplay-campaign-worker.ts')], { env: { ...process.env, HEURISTIC_PROFILE: heuristicProfile.id, CAMPAIGN_AUTOPLAY_SEED: String(entry.seed), CAMPAIGN_AUTOPLAY_PROFILE: profile.id, CAMPAIGN_AUTOPLAY_TURN_LIMIT: String(entry.turnBudget), CAMPAIGN_AUTOPLAY_TRACE: captureTrace ? '1' : '0' }, stdio: ['ignore', 'pipe', 'pipe'] })
  let output = ''
  let errors = ''
  let settled = false
