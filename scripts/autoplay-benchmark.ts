...
const reports = seeds.flatMap(seed => biomes.flatMap(biome => floors.flatMap(floor => modes.map(mode => runAutoplay(newRun(seed, biome, floor), { mode, policy, heuristicProfile, turnLimit, chainAreas, chainFloors })))))
...
console.log(JSON.stringify({ config: { seeds, biomes, floors, modes, policy, heuristicProfile: heuristicProfile.id, turnLimit, chainAreas, chainFloors, fullTrace, summaryOnly }, total: reports.length, clearRate, outcomes, detail }, null, 2))
