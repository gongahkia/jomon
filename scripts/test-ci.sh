...
"${vitest[@]}" src/validation.test.ts
"${vitest[@]}" --exclude src/autoplay.test.ts --exclude src/validation.test.ts
...
if [[ "${SKIP_CAMPAIGN_MATRIX:-0}" != "1" ]]; then npm run test:campaign; fi
