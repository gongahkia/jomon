...
"${vitest[@]}" src/autoplay.test.ts --testNamePattern='^(?!.*(completes the Mine reference run|clears the pressure-detour regression seed|clears the ranged-corridor regression seed|clears the moving-route and mixed-altar regression seed|clears the telegraph-route reversal regression seed|clears the offering-cash regression seed|clears the telegraphed Mine exit regression seed|clears the long telegraph-detour regression seed|clears the rail-tunnel telegraph regression seed|clears the telegraphed guardian-route regression seed)).*$'
