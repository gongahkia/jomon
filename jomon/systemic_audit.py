...
def inspect_world(state):
    ...
    for region_id, region in state.regions.items():
        ...
        if any(key not in region.landmarks for key in ("sanctum_secret", "sanctum_side_stair", "field_upper", "field_lower")) or not any(key.startswith("landform_") for key in region.landmarks):
        ...
        metric.update(containers=len(region.containers), actors=len(actors), histories=len(region.regional_history),
                      landforms=sum(key.startswith("landform_") for key in region.landmarks),
