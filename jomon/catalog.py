...
_EQUIPMENT_TEMPLATE_CONTRACT = {
    ...
    **{f"equipment.ammunition.{ammunition.replace(' ', '_')}.name": () for ammunition in ("arrows", "bolts", "sling stones", "heavy bolts", "javelins", "nets", "shot", "handgonne charges", "pitch pots", "lime pots", "brine pots", "smoke bombs", "pitch bombs", "lime bombs", "brine bombs", "thunder bombs", "resin bombs")},
    **{f"equipment.ammunition.{ammunition.replace(' ', '_')}.description": () for ammunition in ("arrows", "bolts", "sling stones", "heavy bolts", "javelins", "nets", "shot", "handgonne charges", "pitch pots", "lime pots", "brine pots", "smoke bombs", "pitch bombs", "lime bombs", "brine bombs", "thunder bombs", "resin bombs")},
...
_TOPOLOGY_TEMPLATE_CONTRACT.update({
    ...
    **{f"topology.navigation.status.{key}": () for key in ("opened", "marked", "seen")},
    **{f"topology.navigation.unavailable.{key}": () for key in ("marked", "danger", "unreachable", "unknown")},
})
...
_HISTORY_REGIONS = ("hearthford", "greywash", "greenwold", "whitecairn", "dunmire", "rillscar", "marlbank", "frostmere")
_HISTORY_NETWORKS = ("bank-measures", "wreck-and-span", "burn-shelter", "cold-road")
_HISTORY_CONTACTS = ("network-contact-hearthford", "network-contact-marlbank", "network-contact-greywash", "network-contact-rillscar", "network-contact-greenwold", "network-contact-dunmire", "network-contact-whitecairn", "network-contact-frostmere")
...
_HISTORY_TEMPLATE_CONTRACT.update({
    **{f"history.region.{region}.{field}": () for region in _HISTORY_REGIONS for field in ("geology", "climate", "institution_name", "goal", "dispute", "service", "opposition")},
    **{f"history.network.{network}.{field}": () for network in _HISTORY_NETWORKS for field in ("name", "goal", "dispute", "service", "opposition")},
    **{f"history.contact.{contact}.{field}": () for contact in _HISTORY_CONTACTS for field in ("name", "role")},
    **{f"history.tie.{left}.{right}": () for left, right in (("hearthford", "dunmire"), ("hearthford", "marlbank"), ("whitecairn", "frostmere"))},
    **{f"history.value.{value}": () for value in ("crisis.flood", "crisis.fire", "crisis.support_loss", "recovery.shared", "recovery.private", "ecology.wet", "ecology.dry", "exposure.exposed", "exposure.sheltered")},
})
...
_ECOLOGY_TEMPLATE_CONTRACT.update({
    ...
    # IDs but no row in the general actor catalog.  Their current-state
    # wording belongs to the selected ecology presentation all the same.
    **{f"ecology.special.{threat_id}.{field}": ()
       for threat_id in ("wheel-train", "floodgate-claimant")
       for field in ("name", "goal")},
    "ecology.special.floodgate-claimant.capability": (),
})
...
def _dd_template_fields(value: str, path: str) -> frozenset[str]:
    ...
    allowed = {'action', 'actions', 'actor', 'affliction', 'amount', 'approach', 'band', 'biome', 'block', 'boons', 'boss', 'cache', 'camp', 'cap', 'cap_kind', 'card', 'cards', 'cargo', 'choice', 'combat_role', 'completed', 'copies', 'cost', 'costume', 'count', 'courier', 'current', 'curse', 'department', 'depth', 'descendants', 'description', 'destination', 'doctrine', 'draws', 'effects', 'elite', 'encounter', 'enemy', 'energy', 'event', 'facility', 'fight', 'file_a', 'file_b', 'finale', 'forecast', 'formula', 'game', 'guardian', 'hazard', 'hp', 'index', 'infusion', 'item', 'items', 'key', 'kind', 'label', 'layout', 'lead', 'liability', 'light', 'limit', 'losses', 'mark', 'marker', 'mastery', 'maximum', 'mechanic', 'message', 'mission', 'name', 'names', 'next', 'next_stage', 'operation', 'opponent', 'option', 'orders', 'outcome', 'overflow', 'owned', 'owner', 'patron', 'person', 'plan', 'pressure', 'progress', 'rank', 'ranks', 'relationship', 'required', 'resource', 'result', 'role', 'roles', 'room', 'round', 'score_a', 'score_b', 'season', 'side', 'source', 'squad', 'stage', 'status', 'strategy', 'strength', 'stress', 'supplies', 'survivors', 'targets', 'threshold', 'total', 'trigger', 'turns', 'upgrade', 'used', 'values', 'verb', 'wins', 'world', 'x', 'y'}
