...
local function save()
 ...
 local ok,why=Store.save(app.history)
...
local function startNew()
 ...
 app.history=result;app.newRun=nil;app.mapBrowser=nil;app.paused=true;app.accumulator=0;app.saveBlocked=false
...
local function exportMap()
 ...
 elseif love.keyboard.isDown('lshift','rshift') then
  d=Map.fromWorld(app.history.view);scope='viewed terrain ONLY; no people or structures'
 else d=Map.fromWorld(app.history.initial);scope='initial terrain' end
...
function love.load()
 ...
 app.history=h or H.new(G.make(C.seed,C.preset,C.mode))
 ...
 renderer:layout(app);renderer:mapRect(app.history.view)
end
local function export()
 love.filesystem.createDirectory('exports')
 local w=app.history.view;local m=Metrics.measure(w);local stamp=os.time()
 ...
 assert(love.filesystem.write('exports/replay-'..stamp..'.dw',app.history:saveText()))
...
function love.update(dt)
 ...
 for _=1,steps do
  ...
  local alive=W.alive(h.live);local start=love.timer.getTime()
  h:advance(love.timer.getTime);app.tickMs=(love.timer.getTime()-start)*1000
  if W.alive(h.live)<alive then
...
function love.textinput(s)
 if app.newRun and not app.mapBrowser and not app.newRun.imported and s:match('^%d+$') and #app.newRun.seed+#s<=10 then app.newRun.seed=app.newRun.seed..s;app.newRun.preview=nil end
end
...
function love.keypressed(key)
 ...
 if app.newRun then
  ...
  elseif key=='return' or key=='kpenter' then startNew()
  ...
  elseif key=='tab' then
   ...
   if n.preview then n.preview.world.mode=n.mode end
  elseif not n.imported then
   if key=='space' then previewNew()
   elseif key=='backspace' then n.seed=n.seed:sub(1,-2);n.preview=nil
   elseif key=='left' or key=='right' then n.preset=cycle(G.presets,n.preset,key=='left' and -1 or 1);n.preview=nil
   elseif key=='up' or key=='down' then n.layout=cycle(Layouts.names,n.layout,key=='up' and -1 or 1);n.preview=nil
   elseif key=='b' then n.climate=cycle(Biomes.climates,n.climate,1);n.preview=nil
   elseif key=='s' then
    ...
    local size=sizes[at%#sizes+1];n.width,n.height=size[1],size[2];n.preview=nil
   elseif key=='o' then n.openness=cycle({0.35,0.48,0.60},n.openness,1);n.preview=nil
   elseif key=='g' then n.biomeScale=cycle({0.75,1,1.4},n.biomeScale,1);n.preview=nil
   elseif key=='f' then n.features=cycle({'living','ruins','none'},n.features,1);n.preview=nil
   elseif key=='x' then n.density=cycle({0.6,1,1.4},n.density,1);n.preview=nil
   elseif key=='k' then n.crew=cycle({3,6,9},n.crew,1);n.preview=nil end
  end
  return
 end
 local h=app.history
 ...
 elseif key=='y' then
  if love.keyboard.isDown('lshift','rshift') then
   ...
   if cell then local gx,gy=W.tile(h.view,cell.x,cell.y);queue({type='target_order',gx=gx,gy=gy,worker=app.orderWorker or app.selectedWorker or 0}) end
  else
   app.orderWorker=app.selectedWorker~=app.orderWorker and app.selectedWorker or nil
   local a=app.orderWorker and W.find(h.view.workers,app.orderWorker)
 ...
 elseif key=='n' then
  ...
  app.newRun={seed=tostring((h.live.seed+1)%2147483647),preset='frontier',mode=h.live.mode,
   ...
   openness=g.openness or C.openness,biomeScale=g.biomeScale or C.biomeScale,features=g.features or C.features,density=g.density or C.density,crew=g.crew or C.crew,view=1}
 ...
 elseif key=='=' or key=='+' or key=='-' then
  ...
  local cell=app.selectedCell or app.hover
  if cell then local gx,gy=W.tile(h.view,cell.x,cell.y);queue({type='priority',gx=gx,gy=gy,value=app.priority}) end
  notify('Order priority '..app.priority)
 elseif key=='i' or key=='o' or key=='t' then
  local cell=app.selectedCell
  local s=cell and W.structureAt(h.view,cell.x,cell.y)
  if s and s.kind=='charge' and key=='t' then queue({type='arm',slot=W.slot(h.view,s.gx,s.gy),worker=app.orderWorker or 0,priority=app.priority})
  ...
  elseif s and s.kind=='pump' then
   ...
   else app.port=key=='i' and 'intake' or 'outlet';app.portSlot=W.slot(h.view,s.gx,s.gy) end
  else notify('Select a completed pump; T also operates a ward or orders charge arming.') end
 elseif key=='v' then
  if h.live.mode~='practice' then notify('Material brushes are practice-only.')
...
function love.mousemoved(mx,my,dx,dy)
 if app.crew or app.fieldnotes or app.newRun or app.mapBrowser or app.help then return end
...
function love.mousereleased(mx,my,button)
 ...
 local x,y=renderer:cell(mx,my);if not x then return end
 local ex,ey=W.tile(app.history.view,x,y);local count=0
...
function love.wheelmoved(_,dy) if not app.newRun and not app.mapBrowser and not app.help and not app.crew and not app.fieldnotes then renderer.zoom=U.clamp(renderer.zoom+dy*0.25,0.5,6) end end
