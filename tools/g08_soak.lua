-- Deterministic COS-G08 integration route.  The controlled fixture supplies
-- reachable terrain and ordinary raw resources; every archaeology, analysis,
-- scan, refit, socket, expedition, custody, and arrival transition below uses
-- the production G08/Campaign APIs rather than a completed-state fixture.
...
assert(ticks>=12000 and ticks<=60000,'ticks must be 12000..60000')
...
local remaining=ticks-c.tick;assert(remaining>=0,'Integrated G08 setup exceeded requested tick budget');step(c,remaining);Campaign.validate(c)
...
print(('G08 integrated soak seed=%d ticks=%d save=%d->%d->%d maps=%d systems=%d destinations=%d caches=%d relics=%d trace=%s stableDrive=%s scan=systems2+3 remote=site8 person=%d suit=%d remoteRelic=%s->%s solar=%d regulator=%s metalHome=%d metalCampaign=%d componentHome=%d codec=pass history-replay=focused-G08-F'):format(seed,ticks,boot,scanSave,final,maps,#c.region.systems,#c.sites,#c.relics.caches,#c.relics.items,table.concat(traces,','),Relics.driveStatus(c,1,assert(worker(c,8,personId))),personId,suit.id,remoteRelicAtRecovery,Relics.find(c,6).state,Industry.topology(c.sites[8].world).networks[1].generation,regulator.status,c.travel.accounts.sites[1].consumed.metal,spentMetal,c.travel.accounts.sites[1].consumed.component))
print('G08 relic ledger '..table.concat(roles,', ')..' / drive sockets '..table.concat(d.sockets,',')..' / remote cache4=excavated')
