-- Final P01--G08 deterministic integration route.  This is deliberately a
-- separate harness from the tranche soaks: it carries one modern campaign
-- from ordinary industrial preparation through local-frontier archaeology,
-- analysis, a legitimate scan/refit, and a remote settlement, then leaves the
-- mature result running.  Controlled terrain/resources only shorten setup;
-- loading, custody, work, travel, discovery and arrival use production paths.
...
assert(ticks>=30000 and ticks<=100000,'ticks must be 30000..100000')
...
-- The final cross-feature audit is intentionally validator-based rather than
-- maintaining a second shadow ledger.  It catches duplicate person/relic
-- custody, stale journey actors and late-system save-shape errors together.
