...
- `self-play-sandbox` runs deterministic synthetic draw/discard episodes across four seats and
  ...
  indicators to each other for visible-dora estimates, and the environment exposes a basic
...
- `kenjaku.simulation.environment` exposes `kenjaku-sandbox-environment-v0` state plus
  ...
  Sanma North-as-guest-wind yaku filtering, Tenhou Sanma 1m/9m dora indicator wrap, a basic Sanma
...
- `self-play-sandbox --ruleset tenhou-3p` is not enough to check off the Phase 5 Sanma ruleset
  ...
  action, a basic Kita ron/pass reaction window, and Tenhou's 1m/9m dora indicator wrap only. Real
  Sanma still needs gameplay, exact platform timing, complete call handling, scoring, training, and
  evaluation.
...
9. Use `self-play-sandbox --ruleset tenhou-3p` only to check Sanma sandbox plumbing. Do not mark
   ...
   North guest-wind handling, Kita actions/reactions, and 1m/9m dora wrapping exist. Real
