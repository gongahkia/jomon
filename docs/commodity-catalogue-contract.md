...
The catalogue's own v1 addition changed only internal compiled content. The later cargo-hold and local public-tally contract advance mutable state to v17/Jomon v3 and causal replay to v9 while retaining `FoundationWorld` v15, manifest v6, navigation v1, and IndexedDB layout v4. That state may reference only a closed commodity ID plus bounded lot quantity/condition/status and the one fixed ironwork handoff; it does not copy catalogue definitions or create market, buyer, price, stock, demand, route, location, or broader trade facts. `WorldMarketState.commodityStates` remains the deliberately empty closed placeholder.
