...
Terminal presentation v9 derives one keyboard-first ledger prompt from the validated world: source, current courier, canonical candidate list, availability reason, semantic palette state, paired non-colour cue, accessibility text, and source/time/freshness evidence. At the exact anchor it also carries the compact physical availability/loss readout described in the continuity contract; away from the anchor it carries no such readout. The remappable contextual control opens it; Arrow keys select a bounded candidate, Enter confirms, and Escape cancels with no mutation or time change. The canvas only renders this validated projection and forwards typed intent. Detailed-renderer adapter v3 receives the same prompt/readout bundle but has no input, world, timer, persistence, or authority access.
