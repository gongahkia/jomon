...
`MedievalWorldState` v13 and causal replay projection v5 explicitly encode active courier. `FoundationWorld` remains v14, `WorldDeckNavigationState` remains v1, the immutable manifest and generator/RNG streams are unchanged, and IndexedDB remains layout v4 with no store or migration. A valid v14/v12 full envelope is upgraded read-only: selected initial courier becomes active courier, validated navigation is retained, and its checkpoint is deterministically rebased before full replay validation. Reads never overwrite the old record; an explicit later save may write the current full envelope only after validating its old source/index. Corrupt or ambiguous input remains stored and unavailable.
