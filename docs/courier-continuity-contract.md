...
`FoundationWorld` v15 adds only deterministic static Jomon prop provenance; navigation remains v1, causal-history remains v4, the manifest/generator/RNG stream and immutable household are unchanged, and IndexedDB layout remains v4. Valid v14 sources must first prove their original three-prop envelope before read-only conversion to v15; the mutable continuity contract is otherwise unchanged. Content safety retains the project boundaries against sexual violence, slavery, torture, and harm or endangerment of children.
