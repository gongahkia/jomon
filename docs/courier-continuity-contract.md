...
Mutable state v16 / courier v3 contains fixed `initialCourierId`, optional current `activeCourierId`, canonical ordered `departedCourierIds`, the separate bounded vessel-prop action projection, and bounded cargo lots. Causal replay projection v8 and its typed command retain the confirmation plus canonical continuation or crew-extinction result. Full `FoundationWorld` validation still owns immutable seed/configuration regeneration. State, person, and continuity validation use supplied validated immutable household evidence and strict cross-record checks; they do not add duplicate regeneration.
...
Terminal presentation v13 distinguishes awaiting initial selection, active play, and a source-backed read-only crew-extinction state. At the physical `prop:task-ledger` anchor only, its compact readout lists the canonical household in order as active (with truthful current availability), available, committed, temporarily unavailable, permanently departed, or dead. A retained validated continuity command may state only that the active perspective continued after a recorded permanent loss; it never exposes a cause, location, evidence ID, possession, relationship, memory, health detail, hidden person, or loss control. The deck and detailed-renderer adapter v7 remain adapters with no continuity, persistence, replay, timer, input-execution, or roster authority. This contract introduces no player-triggered death/departure operation, rest, conversation, recruitment, succession choice, or other prop action; the separate cargo hold does not affect courier loss or continuity choice.

`FoundationWorld` v15 adds only deterministic static Jomon prop provenance; navigation remains v1, current causal history is v6/replay v8, and the manifest/generator/RNG stream and immutable household are unchanged. IndexedDB layout remains v4. Valid v14 sources must first prove their original three-prop envelope before read-only conversion to v15; later mutable action/cargo bridges compose after that proof. Content safety retains the project boundaries against sexual violence, slavery, torture, and harm or endangerment of children.
