...
Terminal presentation v8 distinguishes awaiting initial selection, active play, and a source-backed read-only crew-extinction state, while exposing no loss control or individual loss surface. The deck and detailed-renderer adapter remain adapters with no continuity, persistence, replay, timer, input-execution, or roster authority. The next physical vessel surface owns availability/loss display; this contract introduces no player-triggered death/departure operation, rest, conversation, cargo, travel, recruitment, succession UI, or other prop action.
