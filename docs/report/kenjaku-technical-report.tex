...
does not claim playing strength, production readiness, exact scoring, or complete Sanma coverage.
Instead, it documents the current architecture and the evidence that is tracked in the repository:
348 unit tests with 15 skips, fixture-only launch media, a fixture benchmark dashboard, and runbooks
...
kan timing, several yaku metadata paths, abortive draws, and narrow Sanma support, but it is not an
exact fu/han scoring engine and not a complete Tenhou Sanma implementation. The README and status
command intentionally preserve these negative claims.
...
The repository has broad unit-test coverage for current primitives and commands. & 2026-07-07 local run: 348 tests, 15 skipped; command recorded in README launch evidence. & Supported \\
...
Kenjaku has exact scoring and complete Sanma rules. & Open GitHub issues TODO-203 and TODO-401 remain. & Not claimed \\
\bottomrule
\end{tabular}
\end{table}
