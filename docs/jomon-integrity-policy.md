...
`effects-loss-recovery.test.ts` constructs a valid terminal-collapse assessment, verifies that it leaves its real active-world fixture unchanged, and only then explicitly calls the existing `finalizeWorldAsChronicle(world, 'jomon-loss')` owner in the test. The resulting chronicle export is checked as deterministic and parseable. This is coverage of two separate existing contracts, not policy-to-world or policy-to-storage orchestration: production still has no automatic response to an assessment intent.
