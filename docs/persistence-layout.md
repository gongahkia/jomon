...
`FoundationWorld` v14 keeps bounded `WorldDeckNavigationState` v1 inside the same authoritative full envelope. Courier continuity changes only the separately versioned mutable/replay contracts: `MedievalWorldState` is v14 / courier v3 and carries fixed `initialCourierId`, optional mutable `activeCourierId`, and canonical permanent `departedCourierIds`; replay projection is v6. IndexedDB remains layout v4: there is no new store, split record, or database migration. A strict read-only v14/state-v13 or v14/state-v12 conversion accepts only a validated full old envelope, sets active courier equal to selected initial courier, begins departures empty, retains validated navigation, deterministically rebinds the causal checkpoint, and proves current replay. A corrupt, ambiguous, stale, or incompatible old envelope is neither repaired in place nor overwritten; repository reads leave it stored and fail closed. A successful later normal save may write the current full v14 envelope and refreshes only validated derived index/snapshot metadata.
