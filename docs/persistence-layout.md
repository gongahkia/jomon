...
`FoundationWorld` v15 keeps bounded `WorldDeckNavigationState` v1 inside the same authoritative full envelope. It adds only the deterministic canonical eight-prop `FoundationJomon` provenance. Courier continuity changes only the separately versioned mutable/replay contracts: `MedievalWorldState` is v14 / courier v3 and carries fixed `initialCourierId`, optional mutable `activeCourierId`, and canonical permanent `departedCourierIds`; replay projection is v6. IndexedDB remains layout v4: there is no new store, split record, or database migration. A strict read-only v14-to-v15 conversion accepts only a fully validated original three-prop v14 envelope, deterministically adds its five fixed static props, and proves current replay/state validation before returning a v15 value. Existing strict v14/state-v13 or v14/state-v12 conversion still precedes that final static-prop proof. A corrupt, ambiguous, stale, or incompatible old envelope is neither repaired in place nor overwritten; repository reads leave it stored and fail closed. A successful later normal save may write the current full v15 envelope and refreshes only validated derived index/snapshot metadata.
...
Terminal presentation v11 and detailed-renderer adapter v5 add only discardable source-backed physical station-readout data after a world is validated. The readouts are neither envelope fields nor browser persistence records: they create no mutable prop state, store, index, snapshot, migration, layout change, or read-time rewrite.
