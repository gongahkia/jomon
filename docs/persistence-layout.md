...
`FoundationWorld` v15 keeps bounded `WorldDeckNavigationState` v1 inside the same authoritative full envelope. It adds only the deterministic canonical eight-prop `FoundationJomon` provenance. Mutable `MedievalWorldState` v20 / courier v3 carries fixed `initialCourierId`, optional mutable `activeCourierId`, canonical permanent `departedCourierIds`, Jomon v3's exact eight-record latest-action projection, bounded cargo-hold lots, the one-record local public-tally contract, the seed-derived Hearthford Mill Lease worksite result and derived local market bands, plus one authored `ExpeditionState`. That state contains only the first Hearthford slice's loadout/support, map position, objective/resource, hound, reed screen, pressure inputs, injury, and consequence; it creates no second persistence authority, causal history, or replay subsystem. Causal history v7 and replay projection v9 prove the existing journalled mutable results; the expedition remains ordinary bounded envelope state. IndexedDB remains layout v4: there is no new store, split record, or database migration. A strict read-only v14-to-v15 conversion accepts only a fully validated original three-prop v14 envelope, deterministically adds its five fixed static props, and proves current replay/state validation before returning a v15 value. The v15/state-v14 bridge first validates and replays the original legacy source before deriving its bounded action state; the v15/state-v15 cargo bridge then validates/replays that original no-cargo source before deriving an empty Jomon v3 cargo projection; the v15/state-v16 settlement bridge fully validates and replays that original no-contract source before deriving the canonical offered tally contract; the v18-to-v19 bridge derives only the seed-bound unresolved worksite and its v3 market; the v19-to-v20 bridge adds the empty authored expedition state. Earlier state-v13/state-v12 conversion composes before those proofs. A corrupt, ambiguous, stale, or incompatible old envelope is neither repaired in place nor overwritten; repository reads leave it stored and fail closed. A successful later normal save may write the current full v20-state envelope and refreshes only validated derived index/snapshot metadata. The separate compiled commodity catalogue v1 remains compiled content rather than an envelope copy.
...
Terminal presentation v15 and detailed-renderer adapter v8 project source-backed physical station readouts, the local public-tally and Mill Lease worksite prompts, and authoritative latest vessel-prop action status/message feedback after a world is validated. The bounded cargo lots, action records, one bounded contract, and worksite resolution live only inside the existing full envelope; they add no store, index, snapshot format, migration, layout change, or read-time rewrite. A later normal save persists the upgraded envelope through the existing atomic transaction.
