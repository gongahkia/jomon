...
## 2026-09-05 contextual vessel-prompt observation

The chart-table and gangplank reserved prompts advance only the discardable terminal presentation to v10 and the detailed-renderer adapter to v4. The common prompt form reuses the existing terminal-control, deck-plan, and proximity contracts; it adds no mutable/replay field, save record, cache, IndexedDB layout change, or envelope byte. Canonical full-envelope bytes remain 190,844–208,892 initially and 255,791–313,590 after due events, within the existing 512 KiB ceiling.
