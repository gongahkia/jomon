...
Sites connect through physical route records. The bridge chart is an intelligence display, not a teleport menu: it can inspect the current landing and its discovered neighbors, while departure happens by operating an airlock in the landing itself. That airlock materializes a deterministic one-to-five-partition route; its partitions are stitched into one camera-continuous walkable space, with three as the usual length. A surveyed landing reveals adjacent approaches. Route state is saved as a normal run, so leaving and returning does not replace the route with a timed cinematic.
...
Long routes retain a three-partition resident window and shift it as the courier crosses internal seams. Evicted chunks retain compact terrain, actor, item, and prop mutations, so cleared patrols and altered paths do not reset when revisited. Routes have persistent situations (quiet, patrol, hazard, trader, or ecology), and every surveyed link produces a contract lead. Market stock and prices evolve with the sector clock; inactive specialists on the trade routine can improve market availability without changing player cargo.
