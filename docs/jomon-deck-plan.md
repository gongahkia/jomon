...
`createTerminalPresentationModel()` v9 derives one common, renderer-independent terminal map from this validated plan. Its 18 by 8 materialized viewport retains exactly 113 static plan area/hull cells in canonical row/column order and, after zero-time courier selection, overlays one source-backed `@` active-courier marker at the persisted local coordinate. The same projection carries the bounded task-ledger courier-switch prompt and, only at the bound anchor, its six-row physical availability/loss readout with source and current-time evidence. It exposes no raw persistent-person records or hidden world facts. The browser canvas consumes that projection; it owns no deck geometry, glyph character, world fact, roster, or map-state authority. The v1 terminal map legend derives its current visible glyph entries, meanings, accessibility text, and source/time/freshness evidence from those source-backed cells and the closed glyph catalogue; it never receives copied geometry or hidden world knowledge. The v3 detailed-renderer adapter receives the exact map, legend, and prompt/readout projection, including marker, focus, accessibility text, and safety data, and remains a deferred parity adapter, not a detailed renderer.
