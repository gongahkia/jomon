...
Terminal presentation, ASCII glyph data, and the deferred detailed-renderer adapter hold only `JomonPaletteToken` references. They also retain their existing text, ASCII glyph, ARIA, keyboard, and non-colour cues. A colour therefore never becomes the only signal for selection, readiness, warning, risk, or an action. The v10 physical task-ledger readout uses only these existing roles: selected text plus `ACTIVE` for the current courier, ready plus `+` for available, waiting plus `CHOOSE COURIER` for committed, warning plus `WARNING //` for temporary unavailability or permanent departure, and risk plus `ERROR //` for death. The bounded chart-table and gangplank reserved prompts likewise use existing warning text plus `WARNING //` and an explicit disabled Enter label. Their explicit bounded text distinguishes those meanings; no new colour, glyph, or theme control is introduced. This document grants no detailed-renderer implementation and no palette authority to a renderer.
