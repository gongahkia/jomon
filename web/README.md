...
`Autoplay all` advances every seat with that same local policy one visible engine transition at a time. `Stop autoplay` returns Seat 0 to direct control; autoplay also stops at a terminal hand.

`Decision curve` is a local replay audit, not an ELO system. It compares each Seat 0 action to the policy's top legal action at the exact prior timeline snapshot, records heuristic policy loss, classifies it, and links back to the replay frame. It uses no backend and does not claim win probability or game-theoretic EV.
