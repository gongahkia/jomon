...
> Archived upstream documentation, not current Jomon controls or executable
> code. The playable office game opens at the Dullest Dungeon table in Jomon;
> the standalone campaign and its tests live in the sibling repository. Use the
> repository-root README for current launch, controls, and save behavior.
...
Biome worlds use content schema 46 and save version 46. The explicit migration chain supports the recorded version-26/content-20 baseline and versions 27–45; embedded schema-21 through schema-46 rules and the archived content-20 rules remain loadable when their fingerprints match; unavailable rules and unsupported versions are rejected. See the [persistence reference](docs/PERSISTENCE.md).
...
- At the threshold, `A` toggles the highlighted selected crew member's default or advanced package and `N` opens doctrines. Doctrine rules cover movement, mark, wound, guard, stress, control, discard, Death's Door, artillery, casualty recovery, and triage. Their bonuses and liabilities use the same deterministic card-resolution queue, are recorded by source, and cannot activate unless the party meets their broad tag/role requirements. No doctrine is the default.
...
- Any unused biome facility may instead recycle every copy of one cargo item. This spends the facility, adds normal disclosed facility Pressure, and stores at most one of two targeting credits. The next salvage cache consumes one credit to offer its original seeded item plus two deterministic alternatives; all three remain skippable, and the choice cannot request an unoffered item.
...
- A workshop can instead infuse one card copy with one of three deterministic compatible offers from a 16-rule catalog. Infusions alter retention, exhaustion, access, conditional cost, opening order, or one explicitly limited follow-through. Card and deck inspection show the exact marker, rule, and limit. Transformation explicitly removes an infusion.
...
The validator currently reports 25 crew archetypes, 290 technique cards, 50 bounded mastery definitions, 16 infusion rules, 6 curse cards, 129 enemy types, 168 encounter formations, 11 biomes, 6 world types, 30 boons, 30 curses, and 36 stackable items. Each crew member has exactly four schema-23 expansion techniques and two schema-24 signature/rare mastery candidates. These totals are diagnostics, not validity requirements. Rewards are filtered to living classes currently in the expedition.
